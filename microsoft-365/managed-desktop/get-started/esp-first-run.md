...
Para fornecer a experiência ESP, você deve registrar dispositivos no serviço Área de Trabalho Gerenciada da Microsoft de segurança. Para obter mais informações sobre o registro, consulte [Registrar novos dispositivos por](../get-started/register-devices-self.md) conta própria ou Etapas para parceiros [registrarem dispositivos](../get-started/register-devices-partner.md).
...
Área de Trabalho Gerenciada da Microsoft essas configurações no perfil do Piloto Automático usado para os dispositivos dos seus usuários:
...
Área de Trabalho Gerenciada da Microsoft usa essas configurações para a experiência da Página de Status de Registro:
...
> [!NOTE]
> O piloto automático para implantação pré-provisionada no Área de Trabalho Gerenciada da Microsoft está atualmente em visualização pública.
...
2. O administrador de IT inicializa o dispositivo, atinge a experiência fora da caixa de correio e pressiona a tecla Windows tecla cinco vezes.
3. O administrador de IT seleciona Windows Provisionamento do Piloto Automático e seleciona **Continuar**. Na tela Windows configuração do Autopilot, serão exibidas informações sobre o dispositivo.
4. O administrador de IT seleciona **Provisionar** para iniciar o processo de provisionamento.
5. O dispositivo inicia o ESP e passa pelas fases de preparação e instalação do dispositivo. Durante a fase de configuração do dispositivo, você verá a instalação do aplicativo **x de x** exibida (dependendo da configuração exata do perfil ESP).
6. A etapa de configuração da conta atualmente é ignorada na configuração Área de Trabalho Gerenciada da Microsoft, pois desabilitaremos o USER ESP.
...
Se a configuração usada pelo Área de Trabalho Gerenciada da Microsoft não corresponder exatamente às suas necessidades, você poderá registrar um tíquete de suporte por meio do [Portal de Administração](https://portal.azure.com/). Aqui estão alguns exemplos dos tipos de configuração que você pode precisar:

### <a name="autopilot-settings-change"></a>As configurações do piloto automático mudam

Talvez você queira solicitar um modelo de nome de dispositivo diferente. No entanto, não é possível alterar o Modo de Implantação, Ingressar no Azure AD Como, Privacidade Configurações ou Tipo de Conta de Usuário.
...
- Nenhum aplicativo que exige o contexto "usuário" (por exemplo, Teams) pode ser incluído na visualização pública do ESP.
