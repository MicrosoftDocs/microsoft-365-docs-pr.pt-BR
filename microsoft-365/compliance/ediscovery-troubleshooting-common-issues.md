...
1. Conecte-se ao [PowerShell do centro de conformidade & segurança](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell).
...
1. Conecte-se ao [PowerShell do centro de conformidade & segurança](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell) e execute o seguinte comando:
...
Ao executar uma pesquisa de descoberta eletrônica que inclui o SharePoint Online e uma unidade para locais comerciais, você pode receber o erro, `File Not Found` embora o arquivo esteja localizado no site. Este erro estará nos avisos de exportação e errors.csv ou ignorados items.csv. Isso pode ocorrer se o arquivo não puder ser encontrado no site ou se o índice estiver desatualizado. Este é o texto de um erro real (com ênfase adicionada).
...
Uma pesquisa de descoberta eletrônica falha com o erro `recipient not found` . Esse erro pode ocorrer se o objeto de usuário não puder ser encontrado na proteção do Exchange Online (EOP) porque o objeto não foi sincronizado.
...
2. Se você ainda tiver problemas, conecte-se ao [PowerShell do centro de conformidade & segurança](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell) e execute o seguinte comando:

   ```powershell
   Get-ComplianceSearch <searchname> | FL
   ```

3. Encontre a quantidade de dados a serem baixados nos parâmetros SearchResults e SearchStatistics.
...
5. No campo resultados, localize os dados que foram exportados e exiba os erros encontrados.

6. Verifique o arquivo Trace. log localizado no diretório em que você exportou o conteúdo para qualquer erro.
...
2. Conecte-se ao [PowerShell do centro de conformidade & segurança](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell) e execute o seguinte comando:
...
1. Conecte-se ao [PowerShell do centro de conformidade & segurança](https://docs.microsoft.com/powershell/exchange/connect-to-scc-powershell) e, em seguida, execute o seguinte comando para uma retenção de caso de descoberta eletrônica:
