...
- **Marcas.** Use esse filtro para mostrar alertas de uma ou mais marcas de usuário. As marcas são refletidas com base em caixas de correio marcadas ou usuários que aparecem nos alertas. Consulte [Marcas de usuário na ATP do Office 356](..\security\office-365-security\user-tags.md) para saber mais.

- **Origem.** Use esse filtro para mostrar alertas disparados por políticas de alerta no centro de conformidade e segurança ou alertas disparados pelas políticas do Office 365 Cloud App Security ou ambos. Para obter mais informações sobre os alertas do Office 365 Cloud App Security, consulte [Exibindo alertas](#viewing-cloud-app-security-alerts)do Cloud App Security.

> [!IMPORTANT]
> A filtragem e a classificação por marcas de usuário estão atualmente em visualização pública.
> Ele pode ser substancialmente modificado antes de ser lançado comercialmente. A Microsoft não faz garantias, expressas ou implícitas, com relação às informações fornecidas sobre ela.
...
- Os membros do grupo de funções Gerenciador de Descobertas e Descobertas não podem exibir alertas porque nenhuma das funções atribuídas fornece permissão para exibir alertas de qualquer categoria de alerta.
...
|Função|Governança de informações|Prevenção contra perda de dados|Fluxo de emails|Permissions|Gerenciamento de ameaças|Outros|
|:---------|:---------:|:---------:|:---------:|:---------:|:---------:|:---------:|
|Logs de auditoria|||||||
...
- **Exibir detalhes do alerta.** Você pode selecionar um alerta para exibir uma página de sobremenu com detalhes sobre o alerta. As informações detalhadas dependem da política de alerta correspondente, mas geralmente incluem o seguinte: nome da operação real que disparou o alerta (como um cmdlet), uma descrição da atividade que disparou o alerta, o usuário (ou lista de usuários) que disparou o alerta e o nome (e link para) da política de alerta correspondente.
  ...
  - Um link para uma lista de atividades que inclui um item para cada atividade realizada que disparou o alerta. Cada entrada nesta lista identifica quando a atividade ocorreu, o nome da operação real (como "FileDeleted") e o usuário que realizou a atividade, o objeto (como um arquivo, um caso de Descoberta Eletrônica ou uma caixa de correio) em que a atividade foi realizada e o endereço IP do computador do usuário. Para alertas relacionados a malware, isso é links para uma lista de mensagens.
...
Semelhante a um alerta disparado por uma política de alerta no centro de conformidade e segurança, você pode selecionar um alerta do Cloud App Security para exibir uma página de flyout com detalhes sobre o alerta. O alerta inclui um link para exibir os detalhes e gerenciar o alerta no portal do Cloud App Security e um link para a política do Cloud App Security correspondente que disparou o alerta. Consulte [Monitorar alertas no Cloud App Security](https://docs.microsoft.com/cloud-app-security/monitor-alerts).
