...
---
# <a name="set-up-a-connector-to-archive-reuters-fx-data"></a>Configurar um conector para arquivar dados FX da Reuters

Use um conector Globalnet no centro de conformidade do Microsoft 365 para importar e arquivar dados da plataforma FX do Reuters para caixas de correio de usuário em sua organização do Microsoft 365. A Globalnet fornece um conector FX da [Reuters](https://globanet.com/reuters-fx/) configurado para capturar itens da fonte de dados de terceiros (regularmente) e, em seguida, importar esses itens para o Microsoft 365. O conector converte as moedas e taxas FX da conta FX da Reuters em um formato de mensagem de email e importa esses itens para a caixa de correio do usuário no Microsoft 365.

Depois que os dados FX da Reuters são armazenados nas caixas de correio do usuário, você pode aplicar recursos de conformidade do Microsoft 365, como Retenção de Litígio, Descoberta Eletrônico, políticas de retenção e rótulos de retenção e conformidade de comunicação. Usar um conector FX da Reuters para importar e arquivar dados no Microsoft 365 pode ajudar sua organização a manter a conformidade com políticas governamentais e regulamentares.

## <a name="overview-of-archiving-reuters-fx-data"></a>Visão geral dos dados FX do Reuters de arquivamento

A visão geral a seguir explica o processo de uso de um conector para arquivar dados FX da Reuters no Microsoft 365.

![Fluxo de trabalho de arquivamento para dados FX da Reuters](../media/ReutersFXConnectorWorkflow.png)

1. Sua organização trabalha com o FX da Reuters para configurar um site FX da Reuters.

2. Uma vez a cada 24 horas, os itens FX da Reuters são copiados para o site Globalnet Merge1. O conector também converte os itens em um formato de mensagem de email.

3. O conector FX da Reuters criado no centro de conformidade do Microsoft 365 conecta-se ao site Globalnet Merge1 todos os dias e transfere o conteúdo para um local seguro de Armazenamento do Azure na nuvem da Microsoft.

4. O conector importa os itens para as caixas de correio de usuários específicos usando o valor da propriedade *Email* do mapeamento automático de usuário, conforme descrito na [Etapa 3.](#step-3-map-users-and-complete-the-connector-setup) Uma subpasta na pasta Caixa de Entrada chamada **Reuters FX** é criada nas caixas de correio do usuário, e os itens são importados para essa pasta. O conector determina para qual caixa de correio os itens serão importados usando o valor da *propriedade Email.* Cada item FX da Reuters contém essa propriedade, que é preenchida com o endereço de email de cada participante do item.
...
A primeira etapa é  acessar a página Conectores de Dados no Microsoft 365 e criar um conector para os dados FX dos Reuters.

1. Vá até [https://compliance.microsoft.com](https://compliance.microsoft.com/) e clique em **Conectores de dados**  >  **FX do Reuters.**

2. Na página **de descrição do produto FX da Reuters,** clique **em Adicionar conector.**
...
A segunda etapa é configurar o conector FX da Reuters no site Globalnet Merge1. Para obter informações sobre como configurar o conector FX do Reuters, consulte [Merge1 Third-Party Connectors User Guide](https://docs.ms.merge1.globanetportal.com/Merge1%20Third-Party%20Connectors%20Reuters%20FX%20User%20Guide%20.pdf).
...
1. Na página Mapear usuários FX do Reuters para usuários do **Microsoft 365,** habilita o mapeamento automático de usuários.
...
Depois de criar o conector FX do Reuters, você pode exibir o status do conector no centro de conformidade do Microsoft 365.

1. Vá para <https://compliance.microsoft.com/> e clique **em Conectores de dados** na barra de entrada esquerda.

2. Clique na **guia Conectores** e selecione o conector FX do **Reuters** para exibir a página do flyout, que contém as propriedades e informações sobre o conector.
