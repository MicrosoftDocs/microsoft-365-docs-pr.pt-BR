...
---
# <a name="document-metadata-fields-in-advanced-ediscovery"></a>Campos de metadados de documento em Descoberta Avançada

A tabela a seguir lista os campos de metadados para documentos em um conjunto de revisão em um caso em Descoberta Avançada. A tabela fornece as seguintes informações:
...
|Nomes de anexo|AttachmentNames|Attachment_Names|Lista de nomes de anexos.|
...
|Recibo de Entrega de Email||Email_delivery_receipt|Endereço de email fornecido em Headers da Internet para recebimento de entrega.|
...
|EmailReadReceipt*||Email_read_receipt|Endereço de email fornecido nos Headers da Internet para recebimento de leitura.|
...
|Pontuação de relevância da família Problema de caso 1*||Family_relevance_score_case_issue_1|Pontuação de relevância da família Problema de caso 1 de Relevância.|
...
|Pontuação de relevância de arquivo Problema de caso 1*||File_relevance_score_case_issue_1|Pontuação de relevância do arquivo Problema de ocorrência 1 de Relevância.|
|Classe File|FileClass|File_class|Para conteúdo do SharePoint e do OneDrive: **Document**; para conteúdo do Exchange: **Email** ou **Anexo.**|
|ID do arquivo|FileId|File_ID|Identificador de documento exclusivo dentro do caso.|
|Data do sistema de arquivos criada||File_system_date_created|Data criada a partir do sistema de arquivos (só se aplica a dados que não são do Office 365).|
|Data do sistema de arquivos modificada||File_system_date_modified|Data modificada do sistema de arquivos (só se aplica a dados que não são do Office 365).|
...
|Local|Local|Local|Cadeia de caracteres que indica o tipo de local do qual os documentos foram fonte.<br /><br />**Dados importados** - dados que não são do Office 365<br />**Teams** - Microsoft Teams<br />**Exchange** - Caixas de correio do Exchange<br />**SharePoint** - Sites do SharePoint<br />**OneDrive** - Contas do OneDrive|
...
|Marcado como problema de caso marcado previamente 1*||Marked_as_pre_tagged_Case_issue_1|Marcado como problema de caso pré-marcado 1 de Relevância.|
|Marcado como problema de caso de semente 1*||Marked_as_seed_Case_issue_1|Marcado como problema de caso de semente 1 de Relevância.|
...
|Tipo de mensagem|MessageKind|Message_kind|O tipo de mensagem a ser pesquisada. Valores possíveis: contatos docs email **<br /> <br /> <br /> <br /> <br /> externaldata <br /> faxes <br /> im journals reuniões <br /> <br /> <br /> microsoftteams** (retorna itens de chats, reuniões e chamadas no Microsoft Teams) observa postagens **<br /> <br /> <br /> rssfeeds <br /> <br />** tarefas de caixa postal| 
...
|Pontuação de relevância normalizada Problema de caso 1||Normalized_relevance_score_case_issue_1|Pontuação de relevância normalizada Problema de caso 1 de Relevância.|
|Autores do O365||O365_authors|Autor do SharePoint.|
|O365 criado por||O365_created_by|Criado pelo SharePoint.|
|Data do O365 criada||O365_date_created|Data criada a partir do SharePoint.|
...
|ID pai|ParentId|Parent_ID|ID do pai do item.|
...
|Problema de caso de porcentagem de leitura 1||Read_percent_Case_issue_1|Ler por cento Problema de caso 1 de Relevância.|
...
|Problema 1 do grupo de carga de relevância||Relevance_load_group_case_issue_1|Grupo de carga de relevância Problema de ocorrência 1 de Relevância.|
|Descrição do status de relevância Problema de caso 1||Relevance_status_description_Case_issue_1|Descrição do status de relevância Problema de caso 1 de Relevância.|
...
|Comentário de relevância||Relevance_comment|Campo Comentário de Relevância.|
...
|Marcado pelo problema de ocorrência 1||Tagged_by_Case_issue_1|Usuário que marcou esse documento para o Problema de Caso 1 em Relevância.|
...
|To|To|Email_to|Para campo para tipos de mensagem. Format é **DisplayName \<SmtpAddress>**|
...
> [!NOTE]
> Para obter mais informações sobre propriedades pesquisáveis ao pesquisar locais de conteúdo do Office 365 quando você estiver coletando dados para um caso de Descoberta Avançada de Descoberta e, em inglês, consulte Consultas de palavra-chave e condições de pesquisa para Pesquisa de [Conteúdo.](keyword-queries-and-search-conditions.md)