...
## <a name="connect-to-exchange-online-and-security--compliance-center-powershell-in-a-single-session"></a>Conectar-se ao Exchange Online e & Centro de Conformidade do PowerShell em uma única sessão

Antes de executar com êxito o script nesta seção, você precisa baixar e instalar o módulo V2 do PowerShell do Exchange Online. Para obter informações, [consulte Sobre o módulo V2 do PowerShell](/powershell/exchange/exchange-online-powershell-v2#install-and-maintain-the-exo-v2-module)do Exchange Online.
...
Como saber se funcionou? Depois de executar o script, os cmdlets do Exchange & Online e do PowerShell de segurança e segurança são importados para sua sessão de Windows PowerShell local. Se nenhum erro aparecer, você se conectou com êxito. Um teste rápido é executar um cmdlet do Centro de Conformidade e Segurança e & Exchange Online. Por exemplo, você pode executar e **Get-Mailbox** e **Get-ComplianceSearch**.
...
Este exemplo permite que membros do grupo de funções do Gerenciador de Descobertas Eletrônicos pesquisem apenas as caixas de correio de membros do grupo de distribuição Usuários de Ottawa. O Get-DistributionGroup cmdlet no PowerShell do Exchange Online é usado para encontrar os membros do grupo Usuários de Ottawa.
...
Este exemplo permite que os membros do grupo de função personalizado Gerenciadores de Descobertas Do OneDrive pesquisem apenas conteúdo em locais do OneDrive for Business na organização.
...
Este exemplo impede que membros do grupo de função "Gerenciadores de Descoberta do OneDrive" executam ações de pesquisa de conteúdo em qualquer caixa de correio da organização. 
