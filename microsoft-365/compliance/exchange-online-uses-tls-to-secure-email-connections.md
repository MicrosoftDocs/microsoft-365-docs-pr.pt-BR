...
Os servidores do Exchange Online sempre criptografam conexões com outros servidores do Exchange Online em nossos data centers com TLS 1.2. Quando você envia emails para um destinatário que está dentro da sua organização, esse email é enviado automaticamente por uma conexão criptografada usando TLS. Além disso, todos os emails enviados para outros clientes são enviados por meio de conexões criptografadas usando TLS e protegidas com o uso de Sigilo Direto.
...
O novo certificado requer a conexão com os pontos de extremidade da nova AC como parte da validação do certificado. Não fazer isso pode fazer com que o fluxo de emails seja afetado negativamente. Se você proteger seus servidores de email com firewalls que apenas permitem que os servidores de email se conectem a determinados destinos, verifique se o servidor é capaz de validar o novo certificado. Para confirmar se o servidor pode usar o novo certificado, conclua estas etapas:
...
3. Quando o utilitário concluir a verificação, ele retornará um status. Se o status for exibido **OK,** seu servidor de email poderá validar com êxito o novo certificado. Caso não seja, você precisa determinar o que está causando a falha das conexões. Provavelmente, você precisa atualizar as configurações de um firewall. A lista completa de pontos de extremidade que precisam ser acessados incluem:
    - ocsp.globalsign.com
     - crl.globalsign.com
