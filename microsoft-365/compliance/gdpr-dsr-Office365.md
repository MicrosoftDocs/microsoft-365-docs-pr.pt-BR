---
title: Solicitações de Serviços de Dados do Office 365 no RGPD
description: Entenda os direitos do usuário de acordo com o RGPD e como o Office 365 ajuda as empresas a encontrar dados e a agir sobre eles em resposta às DSRs.
keywords: Office 365, DSR, Microsoft 365, Microsoft 365 Education, documentação do Microsoft 365, RGPD
author: BrendaCarter
...
O RGPD (Regulamento Geral sobre a Proteção de Dados) fornece direitos às pessoas (conhecidas na regulamentação como *titulares dos dados*) para gerenciar os dados pessoais que foram coletados por um empregador ou outro tipo de órgão ou organização (conhecido como *controlador de dados* ou apenas *controlador*). Os dados pessoais são definidos muito amplamente no RGPD como quaisquer dados relacionados a uma pessoa identificada ou identificável. O RGPD fornece aos titulares dos dados direitos específicos a seus dados pessoais; esses direitos incluem a obtenção de cópias deles, a solicitação de alterações neles, a restrição do processamento deles, a exclusão deles ou seu recebimento em formato eletrônico para que possam ser movidos para outro controlador. Uma solicitação formal feita por um titular dos dados a um controlador para realizar uma ação em seus dados pessoais é chamada de *Solicitação do Titular dos Dados* ou DSR. O controlador é obrigado a considerar prontamente cada DSR e oferecer uma resposta sólida executando a ação solicitada ou fornecendo uma explicação sobre por que a DSR não pode ser atendida pelo controlador. Um controlador deve consultar seus próprios consultores jurídicos ou de conformidade sobre a disposição adequada de qualquer DSR.

O guia aborda como usar os produtos, os serviços e as ferramentas administrativas do Office 365, de modo que ajudem você a encontrar dados pessoais e agir sobre eles ao responder às DSRs. Especificamente, isso inclui como encontrar, acessar e agir sobre dados pessoais que residem na nuvem da Microsoft. Veja aqui uma breve visão geral dos processos descritos neste guia:

1.  ***Descobrir*** – use as ferramentas de pesquisa e descoberta, como as oferecidas pelo SCC (Centro de Conformidade e Segurança) da Microsoft, para encontrar mais facilmente o conteúdo do cliente que pode ser o objeto de uma DSR. Depois que os documentos potencialmente responsivos são coletados, você pode executar uma ou mais ações de DSR descritas nas etapas a seguir para responder à solicitação DSR. Se preferir, você pode determinar que a solicitação não atende às diretrizes das suas organizações para responder às DSRs.

2.  ***Acessar*** – recupere os dados pessoais que residem na nuvem da Microsoft e, se solicitado, faça uma cópia deles disponibilizando-os para o titular dos dados.

3.  ***Retificar*** – faça alterações ou implemente outras ações solicitadas nos dados pessoais, onde for possível.

4.  ***Restringir*** — restrinja o processamento dos dados pessoais, seja removendo as licenças de vários serviços do Office 365, seja desativando os serviços ou recursos desejados, quando possível. Você também pode remover dados da nuvem da Microsoft e mantê-los no local ou onde preferir.

5.  ***Excluir*** – remova permanentemente os dados pessoais que residem na nuvem da Microsoft.

6.  ***Exportar*** – forneça uma cópia eletrônica dos dados pessoais ao titular dos dados. O "direito de portabilidade dos dados" do RGPD permite que um titular de dados solicite uma cópia eletrônica dos dados pessoais que estejam em um formato estruturado, frequentemente usado e que podem ser lidos por máquinas.
...
[Parte 1: Responder às DSRs para Dados do Cliente](#_Part_1:_Responding) – *Dados do Cliente* são dados gerados e armazenados no Office 365 durante a execução das operações diárias dos seus negócios.[^1] Exemplos dos aplicativos mais frequentemente usados do Office 365 que permitem a você criar dados incluem Word, Excel, PowerPoint, Outlook e OneNote. O Office 365 também consiste em aplicativos como SharePoint Online, Teams e Forms, que permitem a você colaborar melhor com outras pessoas. A Parte 1 deste guia aborda como descobrir, acessar, retificar, restringir, excluir e exportar dados dos aplicativos do Office 365 que foram usados para criar e armazenar dados nos serviços online do Office 365. Ela aborda produtos e serviços para os quais a Microsoft está agindo como um processador de dados na sua organização e, desse modo, o recurso DSR é disponibilizado para seu administrador de locatários.

[Parte 2: Responder às DSRs com relação aos insights gerados pelo Office 365](#_Deleting_documents_in) – o Office 365 fornece determinados insights por meio de serviços como o Delve, MyAnalytics e Workplace Analytics. Como esses insights são gerados e como responder às DSRs relacionadas a eles são explicados na Parte 2 deste guia.

[Parte 3: Responder às DSRs para logs gerados pelo sistema](#_Part_3:_Responding) **–** quando você usa serviços corporativos do Office 365, a Microsoft gera algumas informações como logs de serviço que registram o uso ou o desempenho dos recursos nos serviços online. A maioria dos dados gerados pelo serviço contém identificadores pseudônimos gerados pela Microsoft e essa categoria geralmente é mencionada neste documento como *logs gerados pelo sistema*. Embora esses dados não possam ser atribuídos a um titular de dados específico sem o uso de informações adicionais, alguns deles podem ser considerados pessoais de acordo com a definição do RGPD para "dados pessoais". A Parte 3 deste guia aborda como acessar, excluir e exportar logs gerados pelo sistema.

[Parte 4: Recursos adicionais que auxiliam você com as DSRs](#_Part_4:_Additional) – a Parte 4 deste guia relaciona os cenários limitados nos quais a Microsoft é o controlador de dados quando determinados produtos e serviços do Office 365 são usados.
...
Sua organização do Office 365 pode consistir em ofertas da Microsoft que são uma combinação de serviços baseados em nuvem e produtos de servidor local. Em geral, uma implantação híbrida é o compartilhamento de contas de usuário (gerenciamento de identidades) e recursos (como caixas de correio, sites e dados) que existem na nuvem e localmente. Cenários híbridos comuns incluem:
- Implantações híbridas do Exchange, em que alguns usuários têm caixas de correio locais e outros usuários têm caixas de correio do Exchange Online.
...
As diretrizes para responder às DSRs para Dados do Cliente estão divididas nas quatros seções a seguir.

-   [Usar a ferramenta Descoberta Eletrônica de Pesquisa de Conteúdo para responder às DSRs](#_Using_the_Content)

-   [Usar a funcionalidade no aplicativo para responder às DSRs](#_Using_In-App_functionality)

-   [Responder às solicitações de retificação de DSR](#_Responding_to_DSR)

-   [Responder às solicitações de restrição de DSR](#_Responding_to_DSR_1)
...
Para ajudar você a determinar onde pesquisar dados pessoais ou o que procurar, é útil identificar os aplicativos do Office 365 que as pessoas em sua organização podem usar para criar e armazenar dados no Office 365. Ter conhecimento disso restringe os aplicativos do Office 365 que entram no escopo de uma DSR e ajuda a determinar como você vai pesquisar e acessar dados pessoais relacionados a uma DSR. Especificamente, isso determina se é possível usar a ferramenta Pesquisa de Conteúdo ou se você terá que usar a funcionalidade no aplicativo do aplicativo em que os dados foram criados.

Uma maneira rápida de identificar os aplicativos do Office 365 que as pessoas na sua organização estão usando para criar Dados do Cliente é determinar quais aplicativos estão incluídos na assinatura do Office 365 da organização. Para fazer isso, você pode acessar as contas de usuário no portal de administração do Office 365 e analisar as informações de licenciamento de produto. Consulte <span id="_Set__up" class="anchor"></span>Atribuir licenças aos usuários no Office 365 para empresas.
...
Ao procurar dados pessoais no maior conjunto de dados que a sua organização cria e armazena usando o Office 365, pode ser conveniente considerar, primeiramente, quais aplicativos as pessoas mais provavelmente usaram para criar os dados que você está procurando. A Microsoft estima que mais de 90% dos dados de uma organização que são armazenados no Office 365 são criados no Word, no Excel, no PowerPoint, no OneNote e no Outlook. Os documentos criados nesses aplicativos do Office, mesmo se comprados por meio do Office 365 ProPlus ou de uma licença perpétua do Office, provavelmente são armazenados em um site do SharePoint Online, em uma conta do OneDrive for Business de um usuário ou em uma caixa de correio do Exchange Online de um usuário. Isso significa que é possível usar a ferramenta Descoberta Eletrônica de Pesquisa de Conteúdo para pesquisar (e realizar outras ações relacionadas à DSR) em sites do SharePoint Online, em contas do OneDrive for Business e em caixas de correio do Exchange Online (incluindo os sites e as caixas de correio associados aos Grupos do Office 365, Microsoft Teams, Atribuições de EDU e StaffHub) para encontrar documentos e itens de caixa de correio que podem ser relevantes à DSR que você está investigando. Também é possível usar a ferramenta Pesquisa de Conteúdo para descobrir Dados do Cliente criados em outros aplicativos do Office 365.
...
| | |
| :---: | :---:|
<img src="media/O365-DSR-Doc-Final_image3.png" /><br/>Calendário | <img src="media/O365-DSR-Doc-Final_image4.png" /><br />SharePoint Online |
| <img src="media/O365-DSR-Doc-Final_image5.png" /><br/>Excel |<img src="media/O365-DSR-Doc-Final_image6.png" /><br /> Skype for Business|
| <img src="media/O365-DSR-Doc-Final_image7.png" /><br/>Office Lens |<img src="media/O365-DSR-Doc-Final_image8.png" /><br/> Tarefas |
...
| <img src="media/O365-DSR-Doc-Final_image11.png" /><br />OneNote| <img src="media/O365-DSR-Doc-Final_image12.png" />Tarefa pendente |
| <img src="media/O365-DSR-Doc-Final_image13.png" /><br />Outlook/Exchange Online | <img src="media/O365-DSR-Doc-Final_image14.png" /><br />Vídeo |
| <img src="media/O365-DSR-Doc-Final_image15.png" /><br />Pessoas|<img src="media/O365-DSR-Doc-Final_image16.png" /><br />Visio |
...
-   Caixas de correio do Exchange Online. Isso inclui as caixas de correio associadas aos Grupos do Office 365 e o Microsoft Teams
...
-   Pesquise o log de auditoria do Office 365 em busca de atividades recentes de arquivo e pasta realizadas pelo usuário que é o objeto da DSR. A pesquisa no log de auditoria retornará uma lista de registros de auditoria que conterá o nome e o local dos recursos com os quais o usuário interagiu recentemente. Talvez você possa usar essas informações para criar uma consulta de pesquisa de conteúdo. Consulte [Pesquisar no log de auditoria no Centro de Conformidade e Segurança do Office 365](https://support.office.com/article/Search-the-audit-log-in-the-Office-365-Security-Compliance-Center-0d4d0f35-390b-4518-800e-0c7ec95e946c).

#### <a name="building-search-queries-to-find-personal-data"></a>Criar consultas de pesquisa para encontrar dados pessoais

A DSR que você está investigando provavelmente conterá identificadores que podem ser usados na consulta de pesquisa de palavra-chave para pesquisar os dados pessoais. Veja a seguir alguns identificadores comuns que podem ser usados em uma consulta de pesquisa para encontrar dados pessoais:
...
A DSR que você está investigando provavelmente terá um identificador e outros detalhes sobre os dados pessoais que são o objeto da solicitação e podem ser usados em uma consulta de pesquisa.

Pesquisar apenas por um endereço de email ou uma ID de funcionário provavelmente retornará muitos resultados. Para limitar o escopo da pesquisa para que ela retorne o conteúdo mais relevante à DSR, você pode adicionar condições à consulta de pesquisa. Quando você adiciona uma condição, a palavra-chave e uma condição de pesquisa são conectadas de modo lógico pelo operador Booliano **AND**. Isso significa que apenas os itens que corresponderem a *ambas* (palavra-chave e condição) serão retornados nos resultados da pesquisa.
...
| Tipo de arquivo | A extensão de um documento ou arquivo. Use essa condição para pesquisar documentos e arquivos do Office criados pelos aplicativos do Office 365. Use essa condição ao pesquisar documentos localizados em sites do SharePoint Online e em contas do OneDrive for Business.<br/>Observe que a propriedade do documento correspondente é o tipo de arquivo. <br/>Para obter uma lista completa das extensões de arquivo que podem ser pesquisadas, confira o artigo sobre [extensões de nome de arquivo rastreadas e tipos de arquivo analisados padrão no SharePoint](https://technet.microsoft.com/library/jj219530.aspx).|&nbsp;&nbsp;&nbsp;&bull;&nbsp;&nbsp;csv – pesquisa arquivos CSV (valores separados por vírgula); os arquivos do Excel podem ser salvos no formato CSV e o arquivo CSV pode ser importado facilmente no Excel<br>&bull;&nbsp;&nbsp;docx – pesquisa arquivos do Word <br>&bull;&nbsp;&nbsp;mpp – pesquisa arquivos do Project <br&bull;&nbsp;&nbsp;one – pesquisa arquivos do OneNote <br>&bull;&nbsp;&nbsp;pdf – pesquisa arquivos salvos em um formato PDF <br>&bull;&nbsp;&nbsp;pptx – pesquisa arquivos do PowerPoint <br>&bull;&nbsp;&nbsp;xlxs – pesquisa arquivos do Excel <br>&bull;&nbsp;&nbsp;vsd – pesquisa arquivos do Visio <br>&bull;&nbsp;&nbsp;wmv – pesquisa arquivos de vídeo do Windows Media <br>
| Tipo de mensagem | O tipo de mensagem de email a ser pesquisado. Use essa condição para pesquisar caixas de correio de contatos (Pessoas), tarefas de reuniões (Calendário) ou conversas do Skype for Business. Observe que a propriedade de email correspondente é *tipo*.|&bull;&nbsp;&nbsp;*contatos* – pesquisa a lista Meus Contatos (Pessoas) de uma caixa de correio <br>&bull;&nbsp;&nbsp;*email* – pesquisa mensagens de email <br>&bull;&nbsp;&nbsp;*mensagens instantâneas* – pesquisa conversas do Skype for Business<br>&bull;&nbsp;&nbsp;*reuniões* – pesquisa compromissos e solicitações de reunião (Calendário) <br>&bull;&nbsp;&nbsp;*tarefas* – pesquisa a lista Minhas Tarefas (Tarefas); usar esse valor também retornará tarefas criadas no Microsoft To-Do.<br>|
...
Além de pesquisar dados pessoais em documentos, também é possível usar a Pesquisa de Conteúdo para procurar outros tipos de dados que são criados usando aplicativos nativos do SharePoint Online. Isso inclui dados criados usando listas, discussões e formulários do SharePoint. Quando você executar uma Pesquisa de Conteúdo e pesquisar sites do SharePoint Online (ou contas do OneDrive for Business), os dados em listas, discussões e formulários que corresponderem aos critérios de pesquisa serão retornados nos resultados da pesquisa.

##### <a name="examples-of-search-queries"></a>Exemplos de consultas de pesquisa

Veja a seguir alguns exemplos de consultas de pesquisa que usam palavras-chave e condições para pesquisar dados pessoais em resposta a uma DSR. Os exemplos mostram duas versões da consulta: uma da sintaxe da palavra-chave (em que a condição é incluída na caixa Palavra-chave) e outra mostrando a versão baseada em GUI da consulta com condições.

##### <a name="example-1"></a>Exemplo 1

Este exemplo retorna arquivos do Excel localizados em sites do SharePoint Online e contas do OneDrive for Business que contêm o endereço de email especificado. Observe que os arquivos poderão ser retornados se o endereço de email aparecer nos metadados do arquivo.

***Sintaxe da palavra-chave***

mila@contoso.com E tipo de arquivo="xlxs"
...
Este exemplo retorna arquivos do Excel ou Word localizados em sites do SharePoint Online e contas do OneDrive for Business que contêm a ID ou a data de nascimento especificada do funcionário.
...
Em vez de exportar todos os itens de caixa de correio parcialmente indexados de uma pesquisa, você pode executar novamente uma Pesquisa de Conteúdo para procurar uma lista específica de itens parcialmente indexados e, em seguida, exportá-los. Observe que isso só é possível para itens de caixa de correio. Confira [Preparar um arquivo CSV para uma Pesquisa de Conteúdo direcionada no Office 365](https://support.office.com/article/prepare-a-csv-file-for-a-targeted-content-search-in-office-365-82c97bb4-2b64-4edc-804d-cedbda525d22).


### <a name="next-steps"></a>Próximas etapas

Depois de encontrar os dados pessoais relevantes à DSR, certifique-se de manter a Pesquisa de Conteúdo específica que foi usada para encontrar os dados. Provavelmente você a reutilizará para concluir outras etapas no processo de resposta à DSR, como obter uma cópia dela, exportá-la ou excluí-la permanentemente. [](#_Step_2:_Access)[](#_Step_6:_Export_1)[](#_Step_5:_Delete)
...
Os arquivos dos alunos associados a uma Atribuição são armazenados em uma biblioteca de documentos no site do SharePoint Online do Teams correspondente. Os administradores de TI podem usar a ferramenta Pesquisa de Conteúdo para pesquisar arquivos dos alunos que se relacionam a atribuições. Por exemplo, um administrador pode pesquisar todos os sites do SharePoint Online na organização e usar o nome e a classe do aluno ou o nome da atribuição na consulta de pesquisa para encontrar dados relevantes a uma DSR.

Há outros dados relacionados às Atribuições que não são armazenados no site do SharePoint Online da equipe da classe, o que significa que eles não podem ser descobertos com a Pesquisa de Conteúdo. Isso inclui:
...
O Bloco de Anotações de Classe do OneNote é armazenado no site do SharePoint Online da equipe da classe. Cada aluno em uma classe tem um bloco de anotações privado que é compartilhado com o professor. Também há uma biblioteca de conteúdo em que um professor pode compartilhar documentos com alunos, bem como um espaço de colaboração para todos os alunos na classe. Os dados relacionados a esses recursos podem ser descobertos com a Pesquisa de Conteúdo.

Veja a seguir as diretrizes específicas para pesquisar um Bloco de Anotações de Classe.
...
1. Execute uma Pesquisa de Conteúdo usando os seguintes critérios de pesquisa:
   ...
   - Incluir o nome da equipe da classe como uma palavra-chave de pesquisa; por exemplo, "9C Biology".
...
> caminho: "<https://contosoedu.onmicrosoft.com/sites/9C> Biology/SiteAssets/9C Biology Notebook/" E tipo de arquivo="um"
...
-   Os arquivos anexados a uma reunião são mantidos na reunião real por 180 dias e depois se tornam inacessíveis. Esses arquivos podem ser acessados pelos participantes da reunião ingressando na reunião da solicitação de reunião e, em seguida, exibindo ou baixando o arquivo anexado. Confira a seção "Usar os anexos na reunião" em [Pré-carregar anexos de uma reunião do Skype for Business](https://support.office.com/article/preload-attachments-for-a-skype-for-business-meeting-fd3d9f9d-b448-4754-b813-02e49393f251).
...
-   Se a caixa de correio do Exchange Online de um participante da reunião for colocada em Retenção de Litígio ou atribuída a uma política de retenção do Office 365, os arquivos anexados a uma reunião serão retidos na caixa de correio dos participantes. É possível usar a Pesquisa de Conteúdo para pesquisar esses arquivos na caixa de correio dos participantes se o período de retenção do arquivo não tiver expirado. Para obter mais informações sobre como reter arquivos, confira o artigo sobre [retenção de arquivos grandes anexados a uma reunião do Skype for Business](https://docs.microsoft.com/skypeforbusiness/set-up-policies-in-your-organization/retaining-large-files-attached-to-a-meeting).
...
Há duas maneiras de usar a ferramenta Pesquisa de Conteúdo para obter uma cópia de um item de caixa de correio ou documento que você encontrou após execução de uma pesquisa.
...
4.  Clique em **Baixar arquivo original** para baixar o item em seu computador local. Você também terá que baixar itens que não podem ser visualizados.
...
Você também pode exportar os resultados de uma pesquisa de conteúdo para obter uma cópia de mensagens de email, documentos, listas e páginas da Web contendo dados pessoais, embora esse método seja mais complexo do que a visualização de itens. Confira a próxima seção para obter detalhes sobre como [exportar os resultados de uma Pesquisa de Conteúdo](#_Step_6:_Export_1).
...
- Oferecendo aplicativos do Office 365 que salvam dados em formato eletrônico nativo frequentemente usado, que possa ser lido por máquina. Para obter mais informações sobre formatos de arquivo do Office, confira os [documentos técnicos sobre os formatos de arquivo do Office](https://msdn.microsoft.com/library/office/cc313105(v=office.12).aspx).
...
2.  Na página do submenu de pesquisa, clique em ![](media/o365-dsr_image21.png) **Mais** e em **Exportar resultados**. Observe que também é possível exportar um relatório.
...
6.  Na coluna **Nome**, clique no trabalho de exportação que você acabou de criar. O nome do trabalho de exportação é o nome da pesquisa de conteúdo acrescido de **\_Export**.
...
8.  Na parte superior da página do submenu, clique em ![](media/o365-dsr_image21.png) **Baixar resultados**.
...
Para ver instruções passo a passo detalhadas, confira [Exportar resultados da Pesquisa de Conteúdo do Centro de Conformidade e Segurança do Office 365](https://support.office.com/article/Export-Content-Search-results-from-the-Office-365-Security-Compliance-Center-ed48d448-3714-4c42-85f5-10f75f6a4278).
...
-   **Exclusão temporária vs. exclusão irreversível**: nos serviços do Office 365, como o Exchange Online, SharePoint Online e OneDrive for Business, existe o conceito de *exclusão temporária* e *exclusão irreversível*, que se relaciona à capacidade de recuperação de um item excluído (geralmente por um período limitado) antes de ele ser permanentemente removido da nuvem da Microsoft sem chance de recuperação. Nesse contexto, um item excluído temporariamente pode ser recuperado por um usuário e/ou um administrador por um período limitado antes de ser excluído permanentemente. Quando um item for excluído de maneira irreversível, ele será marcado para remoção permanente e eliminado assim que for processado pelo serviço correspondente do Office 365. Veja a seguir como a exclusão temporária e a exclusão irreversível funcionam para itens em caixas de correio e sites (independentemente de quem os exclui: o proprietário dos dados ou um administrador):

    -   **Caixas de correio:** um item é excluído temporariamente quando é excluído da pasta Itens Excluídos ou quando um usuário o exclui pressionando **Shift + Delete**. Quando o item é excluído temporariamente, ele é movido para a pasta Itens Recuperáveis na caixa de correio. Nesse ponto, o item pode ser recuperado pelo usuário até que o período de retenção do item excluído expire (no Office 365, a política de retenção de item excluído é de 15 dias, mas pode ser aumentado para até 30 dias por um administrador). Depois que o período de retenção expira, o item é excluído de maneira irreversível e movido para uma pasta oculta (chamada *Limpezas*). O item será removido permanentemente (limpo) do Office 365 na próxima vez que a caixa de correio for processada (caixas de correio são processadas uma vez a cada 7 dias).
    -   **Sites do SharePoint Online e OneDrive for Business**: quando um arquivo ou documento é excluído, ele é movido para a Lixeira do site (também chamada de *Lixeira de primeiro estágio* (que é como a Lixeira no Windows). O item permanecerá na Lixeira por 93 dias (o período de retenção de item excluído para sites no Office 365). Após esse período, o item é movido automaticamente para a Lixeira do conjunto de sites, também chamada de *Lixeira de segundo estágio*. (Observe que os usuários ou administradores – com as permissões apropriadas – também podem excluir itens da Lixeira de primeiro estágio.) Nesse ponto, o item se torna excluído temporariamente; ele ainda pode ser recuperado (pelo administrador de um conjunto de sites no SharePoint Online, ou pelo usuário ou administrador no OneDrive for Business). Quando um item é excluído da Lixeira de segundo estágio (manual ou automaticamente), ele se torna excluído irreversivelmente e não pode mais ser acessado por um usuário ou administrador de TI. Observe que o período de retenção é de 93 dias para as lixeiras de primeiro e segundo estágio. Isso significa que a retenção da Lixeira de segundo estágio começa quando o item é excluído pela primeira vez. Portanto, o tempo total mínimo de retenção é de 93 dias para ambas as lixeiras.
...
Observe que não é possível excluir um site que foi colocado em uma retenção de Descoberta Eletrônica ou que foi atribuído a uma política de retenção. Os sites devem ser removidos de uma retenção de Descoberta Eletrônica ou política de retenção para que seja possível excluí-lo.

### <a name="deleting-a-onedrive-for-business-site"></a>Excluir um site do OneDrive for Business

De modo semelhante, você pode decidir excluir o site do OneDrive for Business de um usuário em resposta a uma solicitação de exclusão de DSR. Se você excluir a conta do Office 365 do usuário, o respectivo site do OneDrive for Business será retido (e poderá ser restaurado) por 30 dias. Após os 30 dias, ele será movido para a Lixeira do SharePoint Online (exclusão temporária) e, após 93 dias, ele será excluído permanentemente (exclusão irreversível). Para acelerar esse processo, você pode usar o cmdlet [Remove-SPOSite](https://docs.microsoft.com/powershell/module/sharepoint-online/remove-sposite?view=sharepoint-ps) para mover o site do OneDrive for Business para a Lixeira e usar o cmdlet [Remove-SPODeletedSite](https://docs.microsoft.com/powershell/module/sharepoint-online/remove-spodeletedsite?view=sharepoint-ps) para excluí-lo permanentemente. Assim como os sites do SharePoint Online, não é possível excluir um site do OneDrive for Business de um usuário se ele tiver sido atribuído a uma retenção de Descoberta Eletrônica ou a uma política de retenção antes que a conta do usuário tenha sido excluída.
...
Além dos arquivos criados pelo usuário armazenados em contas do OneDrive for Business e sites do SharePoint Online, esses serviços armazenam informações sobre o usuário que são usadas para permitir várias experiências. Eles foram abordados anteriormente neste documento. Confira a seção [Considerações adicionais para seleção de aplicativos](#_Additional_considerations_for) em [Usar a ferramenta Descoberta Eletrônica de Pesquisa de Conteúdo para responder às DSRs](#_Set__up), para obter informações sobre como acessar, exibir e exportar dados de aplicativo do OneDrive for Business e SharePoint Online.
...
Você pode usar a funcionalidade Ação da Pesquisa de Conteúdo para excluir temporariamente itens que são retornados por uma Pesquisa de Conteúdo. Como explicado anteriormente, os itens excluídos temporariamente são movidos para a pasta Itens Recuperáveis na caixa de correio.
...
Além de excluir dados pessoais em resposta a uma solicitação de exclusão de DSR, o "direito de ser esquecido" de um titular de dados também pode ser cumprido pela exclusão da respectiva conta de usuário do Office 365. Veja a seguir alguns motivos pelos quais você talvez queira excluir um usuário:

-   O titular dos dados saiu (ou está em processo de saída) da sua organização.

-   O titular dos dados solicitou que você exclua logs gerados pelo sistema que foram coletados sobre ele. Exemplos de dados nos logs gerados pelo sistema incluem dados de uso de serviço e aplicativo do Office 365, informações sobre solicitações de pesquisa realizadas pelo titular dos dados, bem como dados gerados por produtos e serviços como um produto da funcionalidade do sistema e interação por usuários ou outros sistemas. Para obter mais informações, confira [Parte 3: Responder às DSRs para logs gerados pelo sistema](#_Part_3:_Responding) neste guia.[](#_Part_3:_Responding)

-   Impedir permanentemente que o titular dos dados acesse ou processe dados no Office 365 (em contraposição ao acesso temporariamente restrito pelos métodos descritos na seção [Responder às solicitações de restrição de DSR](#_Responding_to_DSR_1).[](#_Responding_to_DSR_1)

Depois de excluir uma conta de usuário do Office 365:

-   O usuário não pode mais entrar no Office 365 nem acessar nenhum dos recursos do Office 365 da organização, como a respectiva conta do OneDrive for Business, sites do SharePoint Online ou a respectiva caixa de correio do Exchange Online.
...
-   Alguns aplicativos do Office 365 removerão informações sobre o usuário. Por exemplo, no Microsoft Flow, o usuário excluído será removido da lista de proprietários de um fluxo compartilhado.

-   Os logs gerados pelo sistema sobre o titular dos dados serão excluídos 30 dias depois que a conta de usuário for excluída. Para saber mais, confira [Excluir logs gerados pelo sistema](#_Deleting_system-generated_logs) neste guia.[](#_Deleting_Pseudonymized_Data)

>[IMPORTANTE] Depois de excluir uma conta de usuário, essa pessoa perderá a capacidade de entrar no Office 365 e de entrar em qualquer outro produto ou serviço dos quais ela dependia antigamente para uma conta corporativa ou de estudante. Essa pessoa também não poderá iniciar qualquer solicitação DSR por meio da Microsoft diretamente em instâncias onde a Microsoft é o controlador de dados. Para saber mais, confira a seção [Produtos e serviços autenticados com uma ID de organização para a qual a Microsoft é um controlador de dados](#_Product_and_services), na Parte 4 deste guia.
...
-   Quando você inicialmente exclui uma conta de usuário (excluindo o usuário no centro de administração do Office 365 ou no Portal do Azure), ela é excluída temporariamente e movida para a Lixeira do Azure por até 30 dias. Nesse ponto, a conta de usuário pode ser restaurada.
...
-   **Do 31º ao 60º dia**: por 30 dias, depois que a conta de usuário é excluída irreversivelmente, um administrador da organização pode recuperar os dados na caixa de correio e importá-los em outra caixa de correio. Isso fornece às organizações do Office 365 a capacidade de recuperar os dados da caixa de correio, se necessário.

-   **Do 61º ao 90º dia**: um administrador não pode mais recuperar os dados na caixa de correio. Os dados da caixa de correio serão marcados para remoção permanente, e levará mais 30 dias para que os dados da caixa de correio sejam limpos do Office 365.

Se você determinar que o ciclo de vida dessa caixa de correio não atende aos requisitos da organização para responder a uma solicitação de exclusão de DSR, será possível [contatar o Suporte da Microsoft](https://support.microsoft.com/) * depois* de excluir irreversivelmente a conta do usuário, bem como solicitar a Microsoft que inicie manualmente o processo para remover permanentemente os dados da caixa de correio. Observe que esse processo para remover permanentemente os dados de caixa de correio começa automaticamente após o 61º dia do ciclo de vida, de modo que não há motivo para contatar a Microsoft depois desse ponto no ciclo de vida.
...
||||
|:-----:|:-----:|:-----:|:-----:|
|[<img src="media/O365-DSR-Doc_image35.png"/>](#access)<br/>Access|[]()[<img src="media/O365-DSR-Doc_image22.png" />](#business-apps-for-office-365)<br/>Aplicativos de Negócios <br/>para Office 365 |[<img src="media/O365-DSR-Doc_image22.png"/>](#education)<br/>Educação|
|[<img src="media/O365-DSR-Doc_image24.png" />](#flow)<br/>Flow|<img src="media/formslogo-32px.png" /><br/>Formulários|[<img src="media/O365-DSR-Doc_image28.png" />](#kaizala)<br/>Kaizala|
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Access para encontrar, acessar, exportar e excluir dados pessoais.
...
- Localizar registros em um banco de dados do Access <!-- need links here and below -->
...
- Exportar dados para um documento do Word
- Exportar dados para um arquivo de texto

##### <a name="export"></a>Exportar
Conforme já explicado, você pode exportar dados de um banco de dados do Access para formatos de arquivo diferentes. O formato de arquivo que você escolhe para a exportação pode ser determinado pela solicitação de exportação de DSR específica de um titular de dados. Veja Importar e exportar para obter uma lista de tópicos que descrevem como exportar dados do Access para diferentes formatos de arquivo.

##### <a name="delete"></a>Excluir
Você pode excluir um registro inteiro ou apenas um campo de um banco de dados do Access. A maneira mais rápida de excluir um registro de um banco de dados do Access é abrir a tabela no modo Folha de dados, selecionar o registro (linha) ou apenas os dados em um campo que você deseja excluir e pressionar Delete. Você também pode usar uma consulta de seleção que criou para localizar dados e convertê-la em uma consulta de exclusão. Veja: •   Excluir um ou mais registros de um banco de dados •   Criar e executar uma consulta de exclusão
...
- **Reserva de eventos. ** Trata-se de reuniões entre o proprietário da empresa ou a equipe designada e um cliente, que são criadas pelo proprietário da empresa ou pelo cliente por meio da página de reserva pública do proprietário da empresa. Esses dados incluem nome, endereço, endereço de email, telefone e outras informações que o proprietário da empresa coleta do cliente no momento da reserva.
- **Confirmações/cancelamentos/atualizações por email. ** Trata-se de mensagens de email geradas e enviadas pelo sistema em associação a eventos de reserva específicos. Elas contêm dados pessoais sobre os funcionários que estão agendados para prestar o serviço relevante e contêm dados pessoais sobre o cliente que foram inseridos pelo proprietário da empresa ou pelo cliente no momento da reserva.
...
Se um cliente decide deixar o serviço, o conteúdo do cliente é excluído após 90 dias. Para saber mais sobre quando o conteúdo da caixa de correio é excluído depois que a conta um usuário é excluída, confira [Remover dados do Exchange Online](#removing-exchange-online-data).

#### <a name="end-user-identifiable-information"></a>Informações de identificação de usuário final

EUII (Informações de Identificação do Usuário Final) incluem informações pessoais e de contato sobre a equipe que é agendada no Bookings. São adicionadas às páginas de Detalhes da equipe quando o proprietário da empresa configura o Bookings e faz atualizações após a instalação. Contêm o nome, as iniciais, o endereço de email e o número de telefone do membro da equipe. Esses dados são armazenados na caixa de correio do Exchange Online que hospeda o Bookings.

Esses dados são mantidos enquanto o membro da equipe está ativo no serviço, a menos que ele seja excluído explicitamente pelo proprietário da empresa ou por um administrador usando a interface do usuário no aplicativo ou excluindo a caixa de correio de reserva relevante. Quando o administrador inicia a exclusão dos detalhes da equipe ou o membro da equipe deixa o serviço, os detalhes são excluídos de acordo com as políticas de retenção do conteúdo de caixa de correio do Exchange Online definidas pelo proprietário da empresa ou pelo administrador.

##### <a name="discoveraccess"></a>Descobrir/Acessar
O Bookings reúne e armazena os seguintes tipos de dados:
- Informações de perfil de negócios. O conteúdo do cliente sobre a empresa que está usando o Bookings é coletado por meio do formulário de Informações de Negócios do Bookings e sincronizado com o Perfil de Negócios do Centro de empresas se um cliente está usando o Bookings em conjunto com o Centro de empresas.
- O único EUII associado a esses dados é um endereço de email, o C1. Esse endereço é para onde emails de atualização e notificações de novas reservas são enviados.
...
- Eventos de reserva. Trata-se de reuniões do cliente e o conteúdo do cliente relacionado criado pela empresa usando um cliente da Web ou um aplicativo para iOS/Android ou criado pelo cliente usando uma página de reserva pública (ou uma página do Facebook). Os eventos podem incluir nome, endereço, endereço de email, número de telefone e detalhes de compromisso.
...
- **Informações de perfil de negócios e contatos. ** Você pode excluir a caixa de correio do Bookings no centro de administração do Office 365. Depois de excluir a caixa de correio, você pode restaurá-la durante 30 dias. Após 30 dias, a conta e a caixa de correio correspondente são permanentemente excluídas. Para saber sobre a exclusão de uma conta de usuário, confira [Excluir um usuário](#office-365-data-subject-requests-for-the-gdpr).
...
- **Reserva de eventos. ** Você pode excluir reservas de eventos do calendário do Bookings, o que removerá as informações do cliente.
- **Solicitações de reunião, confirmações/cancelamentos/atualizações e lembretes por email. ** Você pode excluí-los do calendário do Bookings, o que removerá as informações do cliente.

Os administradores e os proprietários de empresas também podem excluir dados do cliente usando o portal de privacidade do Centro de empresas. Veja [Exportar ou excluir dados de usuário usando o portal de privacidade do Centro de empresas](https://support.office.com/article/export-or-delete-user-data-using-business-center-privacy-portal-eb48e2c1-4c91-4421-988d-5de497d1e8d8).

Além disso, você pode excluir dados de funcionários e do proprietário da empresa e pode excluir a conta de usuário correspondente do Office 365. Veja [Excluir um usuário](#deleting-a-user).

#### <a name="listings"></a>Listings

As seções a seguir explicam como usar a funcionalidade de app no Microsoft Listings para encontrar, acessar, exportar e excluir dados pessoais.
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Connections para encontrar, acessar, exportar e excluir dados pessoais.

##### <a name="discover"></a>Descobrir
O Conexões coleta e armazena os seguintes tipos de dados: 
- Clientes/contatos são criados pela empresa usando o cliente da Web ou o aplicativo móvel (iOS e Android) ou usando o aplicativo quando uma campanha de marketing por email é enviada a um contato de negócios. Os dados de clientes podem incluir nome, endereço, endereço de email e números de ID fiscal. Observe que os contatos são compartilhados em todos os aplicativos do Centro de empresas.
...
##### <a name="delete"></a>Excluir
Depois que um proprietário do Connections envia uma campanha de email, ele não pode excluir a campanha. Se houver campanhas de rascunho que ele deseje excluir, poderá entrar no painel do Connections e excluir as campanhas de rascunho.

#### <a name="outlook-customer-manager"></a>Outlook Customer Manager
As seções a seguir explicam como usar a funcionalidade no aplicativo do Outlook Customer Manager para encontrar, acessar, exportar e excluir dados pessoais.
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Invoicing para encontrar, acessar, exportar e excluir dados pessoais.

##### <a name="discover"></a>Descobrir
O faturamento coleta e armazena os seguintes tipos de dados:
- **Contatos.** São criados pela empresa quando uma fatura ou estimativa é criada para um cliente/contato de negócios. Os contatos são compartilhados no Centro de empresas. Os dados de cliente incluem nome, endereço, endereço de email e números de ID fiscal.
- **Faturas**. São criadas e enviadas para os clientes e representam uma dívida e uma obrigação fiscal.
- **Estimativas.** A empresa também pode enviar estimativas aos clientes. Se um cliente aceita uma estimativa, ela é convertida em uma fatura. Uma estimativa é convertida em uma fatura depois que é aceita pelo cliente. Registros de estimativas não são mantidos quando elas são convertidas em uma fatura.
...
##### <a name="delete"></a>Excluir
Depois que uma fatura é criada e enviada, não pode ser excluída devido às leis de contabilidade. O proprietário do Faturamento pode solicitar que a Microsoft exclua algumas de suas informações do Office 365 ou todas elas.

Como alternativa, você pode excluir a conta de usuário do proprietário de faturamento no Office 365. Veja [Excluir um usuário](#deleting-a-user).

### <a name="education"></a>Educação
Esta seção explica como usar a funcionalidade no aplicativo dos seguintes aplicativos Microsoft Education para responder a solicitações de DSR.
- Atribuições
- Bloco de Anotações de Classe

#### <a name="assignments"></a>Atribuições
As seções a seguir explicam como usar a funcionalidade no aplicativo do Assignments para encontrar, acessar, exportar e excluir dados pessoais.
...
##### <a name="finding-assignments-data-stored-in-sharepoint----not-flagged-for-header-but-should-be---"></a>Localizar dados de tarefas armazenados no SharePoint <!-- not flagged for header but should be -->
...
Um administrador pode usar a ferramenta Pesquisa de Conteúdo no Centro de Conformidade e Segurança do Office 365 para procurar arquivos de alunos (nas bibliotecas de Trabalho do Aluno e Arquivos da Classe) relacionados a envios em tarefas, bem como arquivos relacionados a tarefas. Por exemplo, um administrador pode pesquisar todos os sites do SharePoint na organização e usar o nome do aluno e o nome da classe ou tarefa na consulta de pesquisa para localizar dados relevantes para uma solicitação de DSR. 
...
Veja a [seção Usando a ferramenta de Descoberta Eletrônica de Pesquisa de Conteúdo para responder a DSRs](#using-the-content-search-ediscovery-tool-to-respond-to-dsrs) neste guia.
...
Observe que, mesmo que um aluno não faça mais parte de uma classe, os dados podem ainda estar presentes na classe e ser marcados como "não registrado". Nesse caso, um aluno que envia uma solicitação de DSR precisa fornecer ao administrador a lista de classes em que foi formalmente registrado.

##### <a name="export"></a>Exportar
Você pode exportar dados de tarefas para um aluno específico para todas as classes em que o aluno está registrado atualmente usando um script do PowerShell. Confira:
- [Usar scripts para exportar e excluir dados de usuários de Atribuições](https://support.office.com/article/04473375-6c00-4dda-bfe5-b55a772958fe).
- [Exportar dados de alunos e professores de tarefas](https://support.office.com/article/e8c36e0e-fb6e-42c5-97fd-66f90f92a13a).
...
##### <a name="delete"></a>Excluir
Você pode excluir dados de Assignments de um aluno específico para todas as classes em que o aluno está atualmente registrado usando um script do PowerShell. Faça isso antes de remover o aluno da classe. Confira:
- [Usar scripts para exportar e excluir dados de usuários de Atribuições](https://support.office.com/article/04473375-6c00-4dda-bfe5-b55a772958fe).
- [Excluir dados de alunos de Tarefas](https://support.office.com/article/a368853a-9565-4320-8e86-2f9423e932a4).
...
A Pesquisa de Bloco de Anotações de Classe para o conteúdo no Bloco de Anotações de Classe é discutida anteriormente neste guia. Veja [Bloco de Anotações de Classe do OneNote](#onenote-class-notebook). Você também pode usar a ferramenta Pesquisa de Conteúdo para exportar dados de um Bloco de Anotações de Classe. Como alternativa, um administrador ou o titular dos dados podem exportar dados de um Bloco de Anotações de Classe. Veja [Salvar uma cópia de um Bloco de Anotações de Classe](https://support.office.com/article/44733e18-0ef1-4d4b-be51-fc2ac5bfe9ec).
...
- [Administrador global do Office 365\](https://support.office.com/article/assign-admin-roles-in-office-365-for-business-eac4d046-1afd-4f1a-85fc-8219c79e1504)
...
3.  Na página **Ambientes**, clique no ambiente dos fluxos que deseja acessar. Observe que as organizações do Office 365 têm um ambiente padrão.
...
Depois de tornar a si mesmo um proprietário, vá para **Fluxo** \> **Meus fluxos** \> **Fluxos da equipe** para acessar o fluxo. No fluxo, você pode baixar o histórico de execuções ou exportar o fluxo. Consulte:
...
-   **Definições do fluxo**: um usuário pode exportar a definição de um fluxo (que é exportada como um pacote do Flow, formatado como JSON em um arquivo compactado). Consulte [Exportar e importar seus fluxos entre ambientes com empacotamento](https://flow.microsoft.com/blog/import-export-bap-packages/).

-   **Históricos de execuções de fluxos**: um usuário pode baixar o histórico de execuções de cada um de seus fluxos. Um histórico de execuções de fluxos é baixado como um arquivo CSV, que pode ser aberto no Excel para filtragem ou pesquisa. Os usuários também podem baixar o histórico de execuções de vários fluxos. Confira [Baixar histórico de execuções de fluxos](https://flow.microsoft.com/blog/download-history-recurrence/).

#### <a name="delete"></a>Excluir

Um administrador pode adicionar a si mesmo como um proprietário dos fluxos de um usuário no centro de administração do Flow. <span id="_Hlk511117287" class="anchor"></span>Se um usuário deixar a sua organização e sua conta do Office 365 for excluída, os fluxos dos quais ele é o único proprietário serão retidos. Isso ajuda a organização a fazer a transição dos fluxos para novos proprietários e a evitar qualquer interrupção nos negócios de fluxos que podem ser usados em processos corporativos compartilhados. Um administrador precisa determinar se exclui os fluxos que pertenciam ao usuário ou simplesmente reatribui a novos proprietários, e executa a ação.
...
-   **Definições do fluxo**: depois que um administrador adiciona a si mesmo como um proprietário de um fluxo, ele pode ir até **Fluxo** \> **Meus Fluxos** \> **Fluxos da equipe** para exportar a definição do fluxo (que é exportada como um pacote do Flow, formatado como JSON em um arquivo compactado). Consulte [Exportar e importar seus fluxos entre ambientes com empacotamento](https://flow.microsoft.com/blog/import-export-bap-packages/).

-   **Históricos de execuções de fluxos**: da mesma forma, um administrador deve adicionar a si mesmo como um proprietário de um fluxo para exportar seu histórico de execuções de fluxos. O histórico de execuções de fluxo é baixado como um arquivo CSV, o que significa que é possível usar o Excel para filtrar ou pesquisar. Também é possível baixar o histórico de execuções de vários fluxos, desde que você tenha propriedade. Confira [Baixar histórico de execuções de fluxos](https://flow.microsoft.com/blog/download-history-recurrence/).
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Forms para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir

Os usuários do Forms podem acessar <https://forms.office.com> e selecionar **Meus formulários** para ver os formulários que criaram. Eles também podem selecionar **Compartilhado comigo** para exibir formulários que outras pessoas compartilharam por um link. Se houver muitos formulários para classificar, os usuários poderão usar a barra de pesquisa no produto para procurar formulários por título ou autor. Para determinar se o Microsoft Forms é o lugar onde provavelmente os dados pessoais responsivos à sua DSR residem, você poderá pedir ao Titular dos Dados para pesquisar sua lista **Compartilhado comigo** a fim de determinar quais usuários ("Proprietários de Formulários") enviaram formulário ao Titular dos Dados. Você pode então pedir que os proprietários de formulários selecionem **Compartilhar** na barra de navegação superior e enviem a você um link para um formulário específico, de modo que seja possível exibi-lo e determinar se ele é essencial à sua DSR.
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Kaizala para encontrar, acessar, exportar e excluir dados pessoais.
...
- Informações de perfil do usuário, além de contatos do usuário
...
#### <a name="access"></a>Acessar
Os usuários do Kaizala podem usar seu dispositivo móvel para ver o conteúdo do Kaizala que você criou em seu dispositivo. Para determinar se os aplicativos móveis do Kaizala são um local onde dados pessoais responsivos a um DSR provavelmente residem, você pode pedir ao titular de dados que pesquise as informações solicitadas no aplicativo do Kaizala.
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Planner para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir

Os planos do Planner são associados a um Grupo do Office 365 e os arquivos dos Grupos do Office 365 são armazenados em um site do SharePoint Online associado do grupo. Isso significa que você pode usar a Pesquisa de Conteúdo para encontrar arquivos do Planner pesquisando o site do Grupo do Office 365. Para isso, você precisará ter a URL do Grupo do Office 365. Confira [Pesquisar Grupos do Office 365 e Microsoft Teams](https://support.office.com/article/content-search-in-office-365-53390468-eec6-45cb-b6cd-7511f9c909e4) no tópico da Ajuda "Pesquisa de Conteúdo no Office 365" para ver dicas de como obter informações sobre os Grupos do Office 365 que ajudem a pesquisar arquivos do Planner no site do SharePoint Online correspondente.

#### <a name="access"></a>Acessar

Conforme explicado anteriormente, é possível pesquisar o site do SharePoint Online subordinado e a caixa de correio que estão associados a um plano. Em seguida, você pode visualizar ou baixar os resultados da pesquisa relacionados para acessar os dados.
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Power BI para encontrar, acessar, exportar e excluir dados pessoais.
...
Você pode imprimir painéis, relatórios e visuais de relatórios em Power BI para produzir uma cópia física. Observe que não é possível imprimir relatórios inteiros; só é possível imprimir uma página por vez. Para fazer isso, vá até um relatório, use o campo de pesquisa para encontrar dados específicos e imprima a página. Confira [Imprimir do serviço do Power BI](https://docs.microsoft.com/power-bi/service-print).
...
Para ajudar a facilitar uma solicitação DSR relacionada ao PowerApps, você pode aproveitar as operações de administração expostas no [Centro de Administração do PowerApps](https://admin.powerapps.com/) e [cmdlets do PowerShell de Administrador do PowerApps](https://go.microsoft.com/fwlink/?linkid=871804). Acessar estas ferramentas exigirá uma conta com as seguintes permissões:

- Uma licença paga de Plano 2 do PowerApps ou uma licença de avaliação de Plano 2 do PowerApps. Você pode assinar uma licença de avaliação de 30 dias [aqui](http://web.powerapps.com/trial).

- [Administrador global do Office 365\](https://support.office.com/article/assign-admin-roles-in-office-365-for-business-eac4d046-1afd-4f1a-85fc-8219c79e1504) ou
...
Depois que você tem acesso ao aplicativo do usuário, é possível usar um navegador da Web para abrir o aplicativo. Depois de abrir um aplicativo, você pode fazer uma captura de tela dos dados. Confira [Usar o PowerApps em um navegador da Web](https://docs.microsoft.com/powerapps/run-app-browser).
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft StaffHub para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir

A maioria dos dados no StaffHub está disponível para todos os gerentes e membros da equipe do StaffHub. Para revisar dados no StaffHub, peça que um gerente ou membro da equipe acesse <https://staffhub.office.com> a fim de procurar dados que sejam potencialmente relevantes à solicitação DSR ou peça que eles adicionem você como um membro da equipe para que seja possível revisar diretamente as informações do StaffHub em busca de dados potencialmente responsivos.

#### <a name="access"></a>Acessar

Depois que conteúdo relevante do StaffHub é encontrado, você pode exibir e fazer capturas de tela de dados relevantes que deseja fornecer ao titular dos dados de dentro do aplicativo ou baixá-los.

-   **Informações sobre agendamento**: a guia **Agenda** no StaffHub oferece um **Modo de Exibição Pessoas** e um **Modo de Exibição Turnos** que permite exibir agendas diárias, semanais e mensais por membros da equipe ou por turnos de trabalho. Essas exibições podem ser impressas ou baixadas para o Excel selecionando os três pontos (reticências) no canto superior esquerdo do aplicativo Web do StaffHub e clicando em **Exportar Agenda**.

-   **Arquivos compartilhados**: todos os arquivos no StaffHub são armazenados no SharePoint Online e, desse modo, podem ser descobertos, acessados e exportados usando a Pesquisa de Conteúdo, como descrito acima. Eles também podem ser exibidos de dentro do StaffHub na guia **Arquivos**.

-   **Mensagens**: atualmente, as mensagens podem ser acessadas seguindo as etapas sobre como acessar dados específicos do usuário no próximo item. Futuramente, todas as mensagens no StaffHub serão armazenadas no Microsoft Teams, o que significa que você poderá usar a ferramenta Pesquisa de Conteúdo para acessá-las.

-   **Dados específicos de usuário**: os dados específicos de usuário consistem em configurações do usuário, feed de atividades do usuário e histórico de solicitações de turno do usuário. Nenhum deles pode ser exibido por administradores ou membros da equipe. Para acessar ou exportar dados específicos de usuário, peça ao titular dos dados que entre na respectiva conta do StaffHub para obtê-los. Como alternativa, se o titular dos dados deixou a organização, você poderá obter esses dados pedindo que o administrador redefina a senha de usuário do titular dos dados a fim de permitir que você ou o administrador entrem na conta em questão.

-   **Kronos**: o StaffHub dá suporte a conexões com o [Kronos](http://www.kronos.com/), que é uma ferramenta de gerenciamento de força de trabalho de terceiros. O StaffHub e o Kronos são processadores independentes e processam os dados da organização em termos individuais que você assinou com cada parte. Se sua DSR se relaciona aos dados mantidos pelo Kronos, será preciso contatar a assistência do Kronos para DSR, e vice-versa.

#### <a name="delete"></a>Excluir

-   A maioria do conteúdo de dados no aplicativo controlada no StaffHub pode ser excluída por um gerente da equipe no próprio aplicativo. Como declarado anteriormente na seção [Descobrir](#discover-5) do StaffHub, você pode adicionar a si mesmo a uma equipe como um gerente de equipe e excluir os dados.[](#discover-5)

-   Quando uma conta de usuário é excluída, o StaffHub redige o número de telefone, o endereço de email e a foto de perfil do usuário dentro de 7 dias, a contar da exclusão da conta. Por padrão, o nome e sobrenome do usuário são retidos para fins de relatórios históricos em algumas exibições de dados compartilhadas, como agendas, relógio de ponto, conversas, solicitações e o feed de atividades (para outros usuários).

#### <a name="export"></a>Exportar
Confira a seção [Acessar](#access-5) seção do StaffHub.[](#access-5)

### <a name="stream"></a>Stream

As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Stream para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir
Para descobrir conteúdo gerado ou carregado para o Stream que pode ser relevante para uma solicitação do titular de dados, um administrador do Stream pode executar um relatório de usuário para determinar quais vídeos, descrições de vídeo, grupos, canais ou comentários, um usuário do Stream pode ter carregado, criado ou postado. Para obter instruções sobre como gerar um relatório, confira [Gerenciamento de dados do usuário no Microsoft Stream](https://docs.microsoft.com/stream/managing-user-data). A saída do relatório está no formato HTML e contém hiperlinks que podem ser usados para navegar até os vídeos de interesse potencial. Se quiser exibir um vídeo que tem permissão personalizada definida e não fizer parte dos usuários originais para os quais o vídeo foi destinado, você poderá exibir no modo de administrador. Confira [Recursos de administração no Microsoft Stream](https://docs.microsoft.com/stream/manage-content-permissions).  
...
### <a name="sway"></a>Sway
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Sway para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir
O conteúdo criado usando o Sway (encontrado em [www.sway.com](http://www.sway.com)) pode ser visto apenas pelo proprietário e por aqueles que o autor deu permissão para exibir o Sway. Confira [Configurações de privacidade no Sway](https://support.office.com/article/privacy-settings-in-sway-394b551c-be6f-4bd7-a70a-f318d72bf217). Para determinar se o Sway é um local onde provavelmente dados pessoais responsivos à sua DSR residem, você pode pedir ao Titular dos Dados e aos usuários organizacionais que provavelmente têm conteúdo gerado sobre o Titular dos Dados para pesquisar seus Sways e compartilhar com você todos os Sways que provavelmente contêm dados pessoais responsivos à solicitação do Titular dos Dados. Para obter informações sobre como compartilhar um Sway, confira "Compartilhar um Sway da sua Conta Organizacional" no artigo [Compartilhar o Sway](https://support.office.com/article/share-your-sway-1cf853b8-ef7e-46b0-b704-003e58d28998).
...
Para saber como excluir um Sway, vá para a seção "Como excluo meu Sway?" em [Configurações de privacidade do Sway](https://support.office.com/article/privacy-settings-in-sway-394b551c-be6f-4bd7-a70a-f318d72bf217).
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Whiteboard para encontrar, acessar, exportar e excluir dados pessoais.

- Whiteboard 2016 no Surface Hub
- Whiteboard em outras plataformas
...
##### <a name="discover"></a>Descobrir
Os arquivos do Whiteboard (.wbx) são armazenados no conta do OneDrive for Business dos usuários. Você pode perguntar ao titular de dados ou a outros usuários se os quadros de comunicações que eles criaram podem conter dados pessoais em resposta a uma solicitação de DSR. Eles podem compartilhar um quadro de comunicações com você, ou você pode obter uma cópia dele para dar ao titular dos dados.

Para acessar e transferir quadros de comunicações: 
1. Dê a si mesmo acesso à conta do OneDrive for Business do usuário. Veja a seção "Obter acesso aos documentos de OneDrive for Business do ex-funcionário" em [Obter acesso e fazer backup dos dados de um usuário antigo](https://support.office.com/article/get-access-to-and-back-up-a-former-user-s-data-a6f7f9ad-e3f5-43de-ade5-e5a0d7531604).
2. Vá para a pasta Dados de Aplicativos do Whiteboard na conta do OneDrive for Business do usuário e copie os arquivos .wbx dos quadros de comunicações que você deseja transferir.
3. Dê a si mesmo acesso à conta do OneDrive for Business do titular de dados e acesse a pasta Dados de Aplicativos do Whiteboard.
4. Cole os arquivos .wbx que você copiou na etapa anterior.

##### <a name="access"></a>Acessar
Se encontrar dados pessoais em um quadro de comunicações que estiver respondendo a uma solicitação de acesso do DSR, você poderá fornecer ao titular dos dados o acesso a um quadro de comunicações de várias maneiras:
...
1. Dê a si mesmo acesso à conta do OneDrive for Business do titular de dados. Veja a seção "Obter acesso aos documentos de OneDrive for Business do ex-funcionário" em [Obter acesso e fazer backup dos dados de um usuário antigo](https://support.office.com/article/get-access-to-and-back-up-a-former-user-s-data-a6f7f9ad-e3f5-43de-ade5-e5a0d7531604)
...
As seções a seguir explicam como usar a funcionalidade no aplicativo do Microsoft Yammer para encontrar, acessar, exportar e excluir dados pessoais.

#### <a name="discover"></a>Descobrir

No centro de administração do Yammer, um administrador verificado pelo Yammer (administrador global do Office 365 ou administrador verificado configurado no Yammer) pode exportar dados pertencentes a um determinado usuário. A exportação inclui as mensagens e os arquivos postados e modificados pelo usuário. Quando uma exportação de dados específica de usuário é executada, o administrador também recebe uma mensagem na caixa de entrada com os dados de atividade da conta do usuário que ele pode fornecer ao usuário se for de sua escolha. Para obter instruções detalhadas, confira [Yammer Enterprise: privacidade](https://support.office.com/article/enterprise-privacy-yammer-eae49f12-4661-4ba5-aa72-01248f0709bf).

As exportações específicas de usuário são para uma única rede, de modo que se o usuário estiver em uma rede externa do Yammer, o administrador deverá exportar dados para essa rede externa, bem como para a rede doméstica.

Para acessar dados não incluídos na exportação de dados, capturas de tela podem ser feitas para perfil do usuário, configurações, associações de grupos, mensagens marcadas com indicador, usuários seguidos e tópicos seguidos. Os usuários ou administradores podem coletar essas informações. Para obter mais informações, confira [Yammer Enterprise: privacidade](https://support.office.com/article/enterprise-privacy-yammer-eae49f12-4661-4ba5-aa72-01248f0709bf).
...
-   O local e o nome do arquivo (para documentos e outros arquivos) do item que precisa ser alterado; localizar os dados em questão faz parte do processo de descoberta, que é explicado na Etapa 1.[](#_Step_1:_Discover)
...
-   [Gerenciar administradores para um conjunto de sites](https://support.office.com/article/Manage-administrators-for-a-site-collection-9A7E46F9-3FC4-4297-955A-82CB292A5BE0)
...
Um administrador global do Office 365 pode acessar a conta do OneDrive for Business de um usuário usando o centro de administração do Office 365.
...
Um administrador global do Office 365 pode atribuir a si mesmo as permissões necessárias para abrir e editar (ou excluir) itens na caixa de correio de outro usuário, como se fosse o proprietário dela. Um administrador global também pode atribuir essas permissões a outro usuário. Especificamente, o administrador global precisa adicionar a permissão **Ler e gerenciar**, que é a permissão de Acesso Completo no Exchange Online. Para obter detalhes, confira:

-   [Conceder permissões de caixa de correio para outros usuários no Office 365](https://support.office.com/article/Give-mailbox-permissions-to-another-user-in-Office-365-Admin-Help-1DBCF12F-A9DE-4D1D-B0B3-A227F8A736D8)
...
<span id="_Restrict" class="anchor"></span>Os proprietários de dados ou administradores IT podem fazer alterações em documentos, listas e páginas do SharePoint Online. Lembre-se destes pontos ao fazer alterações no conteúdo do SharePoint:

-   A atualização de um documento salvará uma nova versão do documento, que conterá a revisão. As versões anteriores do documento não serão atualizadas. Isso significa que é possível que os dados que são o objeto de uma solicitação de retificação de DSR podem persistir em versões antigas do tópico. Observe que as versões antigas de um tópico podem ser excluídas e depois permanentemente removidas do Office 365. Confira a seção [Excluir documentos no SharePoint Online e OneDrive for Business](#_Deleting_documents_in_1) neste guia.[](#_Deleting_documents_in_1)[](#_Deleting_documents_in)

-   Para terminar de redigir um arquivo do SharePoint de maneira que remova do arquivo todos os rastros de um titular dos dados, inclusive todas as versões do arquivo e todas as atividades registradas realizadas pelo titular dos dados, você precisa executar as seguintes etapas:

    1.  Baixe uma cópia do arquivo no computador local.

    2.  Exclua permanentemente o arquivo do SharePoint Online, excluindo o arquivo e, em seguida, excluindo-o da Lixeira de primeiro estágio e da Lixeira de segundo estágio. Confira a seção [Excluir documentos no SharePoint Online e OneDrive for Business](#_Deleting_documents_in_1) neste guia.[](#_Deleting_documents_in)
...
-   Os dados nas listas do SharePoint podem ser editados. Confira [Adicionar, editar ou excluir itens de lista](https://support.office.com/article/add-edit-or-delete-list-items-a4b31f53-f044-470e-9823-4526594bacde).
...
-   Retifique as propriedades do usuário no Centro de Administração do Office 365. As alterações feitas nos dados da conta serão refletidas automaticamente nas experiências do OneDrive for Business e SharePoint Online. Para obter informações, confira [Adicionar ou alterar informações do perfil de um usuário no Azure Active Directory](https://go.microsoft.com/fwlink/?linkid=864809). Observe que para propriedades originadas no Office 365, nenhuma alteração pode ser feita no SharePoint.
...
Observe que isso não afetará todas as experiências, o que pode reter as informações antigas. Por exemplo, o nome do usuário como texto no documento.
...
-   O perfil do usuário:
    ...
    - Se o usuário tiver uma identidade no Office 365, o perfil de usuário no Yammer será extraído automaticamente do Office 365, que obtém as informações de perfil do AAD (Azure Active Directory). Os usuários do Yammer podem alterar temporariamente os respectivos perfis no Yammer, mas essas alterações são substituídas quando há uma alteração no AAD, de modo que o melhor lugar para exibir e alterar dados de diretório é o AAD. O usuário precisará solicitar que o AAD seja atualizado. Confira [Gerenciar usuários do Yammer em seu ciclo de vida do Office 365](https://support.office.com/article/Manage-Yammer-users-across-their-life-cycle-from-Office-365-6c4c8fff-6444-404a-bffc-f9da0bcc3039) e [Adicionar ou alterar informações de perfil de um usuário no Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-users-profile-azure-portal).
...
<span id="_Delete" class="anchor"></span>Veja a seguir como restringir o processamento de dados no Office 365:
...
Se, posteriormente, sua organização determinar que uma restrição não se aplica mais, você poderá encerrar a restrição revertendo as etapas que executou para ativá-la, como reatribuir licenças, reativar um serviço ou permitir que um usuário entre no Office 365.

### <a name="removing-the-license-for-an-office-365-application"></a>Remover a licença de um aplicativo do Office 365

Conforme explicado anteriormente, as licenças para todos os aplicativos do Office 365 que estão incluídas na assinatura do Office 365 da sua organização são atribuídas a todos os usuários por padrão. Se for necessário restringir o acesso a dados que estão sujeitos a uma DSR, um administrador de TI pode usar o portal de administração do Office 365 para desativar temporariamente a licença de um aplicativo do usuário. Se o usuário tentar usar esse aplicativo, ele receberá uma notificação de produto não licenciado ou uma mensagem informando que ele não tem mais acesso. Para obter detalhes, confira [Remover licenças de usuários no Office 365 para empresa](https://support.office.com/article/Remove-licenses-from-users-in-Office-365-for-business-9B497C85-D0A4-4735-80FA-D3565BC05BD1).

**Observações:**


-   Para restringir o acesso de um usuário ao Yammer, primeiramente, você deve [impor a identidade do Office 365 para um usuário do Yammer](https://support.office.com/article/Enforce-office-365-identity-for-Yammer-users-008f940b-6bec-47fc-bcc6-9c6133467562) e, em seguida, remover a licença do Yammer do usuário.
...
Como a maioria dos documentos do Office está localizada em um site do SharePoint Online ou OneDrive for Business, há um processo de alto nível para remover documentos de sites e depois reimportá-los.

1.  Obtenha uma cópia do documento que é o objeto da solicitação de restrição. Talvez seja preciso solicitar acesso ao site, ou pedir que um administrador global do Office 365 ou um administrador de conjunto de sites forneça uma cópia do documento.
...
3.  Exclua permanentemente (limpe) o documento original do Office 365. Esse é um processo de 3 etapas:
    ...
    c. Vá para a Lixeira do conjunto de sites e exclua essa cópia do documento, o que a remove permanentemente do Office 365. Confira [Excluir itens da lixeira do conjunto de sites](https://support.office.com/article/Delete-items-from-the-site-collection-Recycle-Bin-dd5c00c2-aef6-4458-9d04-80b185077653).
...
[Importante] O procedimento anterior não funcionará se o documento estiver localizado em um site que está em retenção (com um dos recursos de retenção ou retenção legal do Office 365). No caso em que uma solicitação de restrição para uma DSR tiver precedência sobre uma retenção legal, a retenção terá que ser removida do site para que um documento possa ser permanentemente excluído. Além disso, o histórico de documentos para documentos excluídos será permanentemente removido.
...
-   **Dados do perfil**. Essas são as informações de perfil da Lista de Endereços Global da sua organização no Azure Active Directory, bem como informações opcionais sobre si mesmos que os usuários optaram por adicionar. Para acessar ou exportar dados do perfil no Delve, um usuário pode clicar em **Eu** \> **Atualizar perfil**. Ele pode copiar o conteúdo diretamente da página ou fazer uma captura de tela.

-   **Dados do blog**. Essas são as postagens de blog publicadas por um usuário. Para acessar ou exportar dados do blog, um usuário pode clicar em **Eu** \> **Todas as postagens**. Ele pode copiar o conteúdo diretamente da página ou fazer uma captura de tela.

-   **Dados de pessoas recentes**. Essas são as pessoas na organização que o Delve deduziu que são as mais relevantes para o usuário em um determinado momento. Quando um usuário clica em **Eu** \> **Ver tudo** no painel "Clicar em uma pessoa para ver no que ela está trabalhando", o Delve mostra as pessoas mais relevantes para um usuário em um determinado momento.
...
Para acessar ou exportar os dados acima, o usuário pode clique no ícone de engrenagem no canto superior direito no Delve e clicar em **Configurações de recurso** > **Exportar dados**. As informações são exportadas em formato JSON.

##### <a name="data-thats-available-through-other-services"></a>Dados que estão disponíveis por meio de outros serviços

-   **Dados de documentos populares**. Esses são documentos e anexos de email que podem ser relevantes para o usuário. O Delve organiza dinamicamente esses documentos e mensagens de email com base nas atividades do usuário e nas pessoas com as quais ele trabalha no Office 365. Quando um usuário abre o Delve ou clica em **Início**, o Delve mostra os documentos ou anexos mais relevantes para o usuário em um determinado momento. Para acessar ou exportar os documentos e anexos reais, o usuário pode acessar o serviço do Office 365 pelo qual o documento ou anexo foi disponibilizado (como Office.com, SharePoint Online, OneDrive for Business ou Exchange Online).

-   **Dados de documentos e anexos de email recentes**. Esses sãos os documentos e anexos de email mais recentes que o usuário modificou. Quando um usuário clica em **Eu** \> **Ver tudo** no painel "Voltar para os documentos e anexos de email recentes", o Delve mostra os últimos documentos e anexos de email que o usuário modificou em um determinado momento. Para acessar ou exportar os documentos e anexos reais, o usuário pode acessar o serviço do Office 365 pelo qual o documento ou anexo foi disponibilizado; por exemplo, Office.com, SharePoint Online, OneDrive for Business ou Exchange Online.

-   **Dados de documentos de pessoas ao seu redor**. Esses são os documentos que o Delve deduziu que são mais relevantes para o usuário em um determinado momento. Quando um usuário clica em **Eu** \> **Ver tudo** no painel "Descobrir documentos de pessoas ao seu redor", o Delve mostra os documentos mais relevantes para um usuário em um determinado momento. Para acessar ou exportar os documentos reais, o usuário pode acessar o serviço do Office 365 pelo qual o documento ou anexo foi disponibilizado (por exemplo, Office.com, SharePoint Online, OneDrive for Business ou Exchange Online).

#### <a name="rectify"></a>Retificar

Os usuários podem modificar as seguintes informações no Delve:

-   **Informações do perfil**. Um usuário pode clicar em **Eu** \> **Atualizar perfil** para atualizar suas informações. Dependendo das configurações da organização na Lista de Endereços Global, os usuários talvez não possam modificar todas as informações do perfil, como nome ou cargo.

-   **Configurações de recurso**. Um usuário pode clicar no ícone de engrenagem, no canto superior direito no Delve, e clicar em **Configurações de recurso** \> para alterar as configurações desejadas.

#### <a name="restrict"></a>Restringir

Para restringir o processamento no Delve em sua organização, você pode desativar o Office Graph. Saiba mais [aqui](https://support.office.com/article/office-delve-for-office-365-admins-54f87a42-15a4-44b4-9df0-d36287d9531b#bkmk_delveonoff).
...
-   **Documentos ou anexos de email**. Para excluir um documento ou anexo, os usuários devem acessar o serviço em que o documento ou anexo está armazenado (como SharePoint Online, OneDrive for Business ou Exchange Online) e excluí-lo.
...
#### <a name="access-and-export"></a>Acessar e exportar
Se sua organização usa o MyAnalytics, a Microsoft gera insights para todos os usuários, quer eles tenham uma licença de usuário do MyAnalytics ou não, para fornecer resultados significativos aos usuários licenciados. Todas as informações do MyAnalytics são derivadas de cabeçalhos de email e reuniões na caixa de correio do usuário. A Microsoft oferece a capacidade de exportar os dados que o MyAnalytics usa para gerar esses insights usando a ferramenta de casos de DSR no Centro de Conformidade e Segurança. Para obter instruções detalhadas, veja [Exportar dados do Microsoft MyAnalytics e do Serviço de Roaming do Office](https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fsupport.office.com%2Farticle%2Fmanage-gdpr-data-subject-requests-with-the-dsr-case-tool-in-the-office-365-security-compliance-center-preview-ce9eb942-3589-42cb-88fd-1576ecb09c5c%3Fstoragetype%3Dstage%23o365datamoreinfo&data=02%7C01%7Ckated%40microsoft.com%7Cab269c3701e84abeaa6808d5c0668501%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636626468936724623&sdata=uJ3kGAa1utTOV06%2FH4YztjbbkDWwfeBCW7obeTQJQ6U%3D&reserved=0).

Além dos dados de insights que você pode exportar usando o Centro de Conformidade e Segurança, os usuários com uma licença do MyAnalytics podem acessar o [painel do MyAnalytics](https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fdelve.office.com%2F%3Fv%3Danalytics&data=02%7C01%7Ckated%40microsoft.com%7Cab269c3701e84abeaa6808d5c0668501%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636626468936724623&sdata=4DW4wNCBbYpJkFUzYX%2B5%2B5nbvLzRaXj%2FUWs02r5bTro%3D&reserved=0) enquanto estão conectados à sua conta do Office 365 para exibir insights gerados sobre como gastam seu tempo no trabalho. Ele poderão criar capturas de tela de insights do MyAnalytics se quiserem ter cópias permanentes das informações.
...
Para restringir o processamento para um usuário específico, você pode optar por retirá-lo do MyAnalytics. Para saber como, confira o artigo sobre [definição das configurações de usuário do MyAnalytics](https://docs.microsoft.com/workplace-analytics/myanalytics/setup/configure-mya-user-settings).
...
O Workplace Analytics permite que as organizações aumentem os dados do Office 365 com seus próprios dados corporativos para obter insights sobre a produtividade organizacional, os padrões de colaboração e o engajamento do funcionário. [Este artigo](https://docs.microsoft.com/workplace-analytics/overview/privacy-and-data-access) explica o controle que a sua organização tem sobre os dados que o Workplace Analytics processa e quem tem acesso a esses dados.

##### <a name="to-assist-you-with-dsrs-in-workplace-analytics"></a>Para ajudar com as DSRs no Workplace Analytics:
Primeiro, determine se sua organização está usando o Workplace Analytics. Para saber mais sobre como fazer isso, confira [Atribuir licenças a usuários no Office 365 para empresas](https://na01.safelinks.protection.outlook.com/?url=https%3A%2F%2Fsupport.office.com%2Farticle%2FAssign-licenses-to-users-in-Office-365-for-business-997596B5-4173-4627-B915-36ABAC6786DC&data=02%7C01%7Ckated%40microsoft.com%7C38fc04499e854ab32b7c08d5c0735793%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636626524040686004&sdata=B%2BiRBXoETxl5TuWQE7J1d0ixLTopyllJDq9MmvLuQl4%3D&reserved=0). Se sua organização não estiver usando o Workplace Analytics, não haverá ação adicional.

Se a organização estiver usando o Workplace Analytics, veja quem em sua organização foi atribuído à função de administrador do Workplace Analytics. Você também deve determinar se a caixa de correio do titular dos dados está licenciada para o Workplace Analytics. Se necessário, peça ao administrador do Workplace Analytics que contate o Suporte da Microsoft para lidar com os DSRs a seguir. 


#### <a name="access-and-export"></a>Acessar e exportar

Os insights nos relatórios do Workplace Analytics criados por você podem ou não conter dados pessoais dos usuários que a sua organização licenciou para o Workplace Analytics, dependendo das informações que a organização usou para suplementar os dados do Office 365. O administrador do Workplace Analytics precisará revisar esses relatórios a fim de determinar se eles contêm dados pessoais de um usuário. Se um relatório não contiver dados pessoais de um usuário, você precisará decidir se deseja fornecer uma cópia desse relatório ao usuário. O Workplace Analytics permite exportar o relatório. 

#### <a name="rectify"></a>Retificar

Conforme explicado acima, o Workplace Analytics usa os dados do Office 365 em combinação com os dados organizacionais que você fornece para gerar relatórios de seu interesse. Os dados do Office 365 não podem ser retificados – eles se baseiam nas atividades de calendário e email de um usuário. No entanto, os dados organizacionais que você carregou no Workplace Analytics para gerar o relatório podem ser retificados. Para fazer isso, será preciso corrigir os dados de origem, carregá-los e executar novamente o relatório para gerar um novo relatório do Workplace Analytics.

#### <a name="restrict"></a>Restringir
Para restringir o processamento de um usuário específico, você pode remover a licença do Workplace Analytics.

#### <a name="delete"></a>Excluir
Se um titular de dados quiser ser removido de um relatório do Workplace Analytics ou um conjunto de relatórios, você poderá excluir o relatório. É sua responsabilidade excluir usuários de quaisquer dados organizacionais usados para gerar o relatório e carregar os dados novamente. Todos os dados sobre o usuário são removidos quando uma conta de usuário é "excluída permanentemente" do Active Directory. 

Para remover os dados pessoais de um assunto de dados, o administrador do locatário executa as seguintes etapas: 

1. Remova a licença do Workplace Analytics do assunto de dados.
...
4. Exporte uma cópia dos dados de RH do sistema de RH da empresa, (confira [Exportar dados](https://docs.microsoft.com/workplace-analytics/setup/prepare-organizational-data#step-three--export-data)), remova as informações do assunto de dados desse arquivo de dados de RH e carregue o arquivo de dados de RH editado no formato. csv no Workplace Analytics (confira [Carregar dados organizacionais](https://docs.microsoft.com/workplace-analytics/setup/upload-organizational-data)).
...
Depois de criar uma solicitação, ela será listada na página **Exportação de Log de Dados**, onde você poderá acompanhar seu status. Após a conclusão de uma solicitação, você poderá clicar em um link para acessar os logs gerados pelo sistema que serão exportados para o local de armazenamento do Azure de sua organização dentro de 30 dias após a criação da solicitação. Os dados serão salvos em formatos de arquivo comuns que podem ser lidos por máquina, como JSON ou XML. Se você não tiver uma conta do Azure e um local de armazenamento do Azure, será preciso criar uma conta do Azure e/ou um local de armazenamento do Azure para a sua organização de modo que a ferramenta Exportação de Log de Dados possa exportar os logs gerados pelo sistema. Para saber mais, confira [Introdução a Armazenamento do Microsoft Azure](https://docs.microsoft.com/azure/storage/common/storage-introduction).

> [!NOTE]
> Quando você criar uma Solicitação de Exportação Dados, dados gerados pelo sistema para alguns aplicativos não serão exportados por meio da ferramenta de Exportação de Log de Dados. Para exportar dados para esses aplicativos, confira [Exportar logs gerados pelo sistema adicionais](https://docs.microsoft.com/microsoft-365/compliance/gdpr-system-generated-log-data).
...
A seguir há um resumo do acesso e da exportação de logs de gerados pelo sistema usando a ferramenta de Exportação de Log de Dados:
  ...
  **Quanto tempo a ferramenta Exportação de Log de Dados da Microsoft leva para concluir uma solicitação?** Isso pode depender de vários fatores. Na maioria das vezes, ela deve ser concluída em um ou dois dias, mas pode levar até 30 dias.

  **Em qual formato será a saída?** A saída será em arquivos estruturados que possam ser lidos por máquinas, como XML, CSV ou JSON.

**Quem tem acesso à ferramenta Exportação do Log de Dados para enviar solicitações de acesso para logs gerados pelo sistema?** Os administradores globais do Office 365 terão acesso ao utilitário Gerenciador de Logs do RGPD.

**Quais dados são retornado pela ferramenta Exportação de Log de Dados?** A ferramenta Exportação de Log de Dados retorna logs gerados pelo sistema armazenados pela Microsoft. Os dados exportados se expandem por vários serviços Microsoft, incluindo Office 365, Azure e Dynamics.

**Como os dados são retornados para o usuário?** Os dados serão exportados para o local de armazenamento do Azure da sua organização. Caberá aos administradores da sua organização determinar como eles mostrarão/retornarão esses dados para os usuários.

**Como serão os dados nos logs gerados pelo sistema?** Exemplo de um registro de log gerado pelo sistema no formato JSON:
...
Um administrador de TI global precisará fazer o seguinte para exportar logs gerados pelo sistema nestas nuvens nacionais:
...
A exclusão permanente de uma conta de usuário removerá os dados do usuário dos logs gerados pelo sistema para quase todos os serviços do Office 365 dentro de 30 dias. A exceção, isto é, quando a exclusão permanente da conta de usuário leva mais de 30 dias, fica por conta do Exchange Online. Dada a natureza crítica do conteúdo do Exchange Online e para impedir a perda acidental de dados, esse sistema foi intencionalmente projetado para colocar os dados em um estado de retenção por até 60 dias depois que uma conta de usuário for permanentemente excluída. Para excluir permanentemente os dados do Exchange Online de um usuário em um prazo de 30 dias, exclua permanentemente a conta do usuário no Azure Active Directory, [contate o suporte](https://support.microsoft.com/) e solicite que os dados do Exchange Online do usuário sejam manualmente removidos do processo de exclusão agendado. Para saber mais, confira [Remover dados do Exchange Online](#removing-exchange-online-data), que foi explicado anteriormente neste guia.

Excluir uma conta de usuário não removerá logs gerados pelo sistema do Yammer e do Kaizala. Para remover os dados desses aplicativos, confira Yammer Enterprise: privacidade e exportação ou excluir dados organizacional do usuário no Kaizala.
...
- Office 365 US Government: envie uma solicitação ao Suporte da Microsoft por meio do [portal de administração do Office 365](http://portal.office365.us).
...
As Partes de 1 a 3 deste guia abrangem produtos e serviços para os quais a Microsoft é um processador de dados da sua organização e, portanto, o recurso DSR é disponibilizado para o administrador de locatários. Há várias circunstâncias em que os usuários da organização podem usar a respectiva conta corporativa ou de estudante (também conhecida como "ID do Azure Active Directory" ou "AAD") para entrar em produtos e serviços da Microsoft para os quais a Microsoft é um controlador de dados. Para todos esses produtos e serviços, os usuários precisam iniciar suas próprias solicitações de titular de dados diretamente com a Microsoft, e a Microsoft tratará as solicitações diretamente com o usuário. Observe que, por natureza, os produtos e serviços que envolvem armazenamento de conteúdo criado pelo usuário permitem que os usuários acessem, exportem, retifique e excluam o respectivo conteúdo como parte da funcionalidade inerente dos produtos. Os cenários em que isso pode se aplicar incluem os seguintes:

- Serviços online opcionais conectados – o Office 365 ProPlus disponibiliza determinados serviços online conectados opcionais para o usuário. A lista desses serviços e os controles de usuário relacionado são listados [aqui](https://support.office.com/article/microsoft-s-other-connected-services-92c234f1-dc91-4dc1-925d-6c90fc3816d8). Você pode decidir se deseja permitir que os usuários finais usem esses serviços. Para saber mais, confira [Como administradores podem gerenciar os serviços de controlador do Office 365 ProPlus](https://docs.microsoft.com/DeployOffice/manage-controller-services-office-365-proplus). Até a extensão em que esses serviços opcionais processam dados pessoais, a Microsoft é um controlador de dados para esses serviços.
- Comentários dos usuários – se os usuários optam por enviar comentários sobre produtos e serviços da Microsoft, a Microsoft é um controlador de dados para esses comentários, na medida em que contêm dados pessoais. A Microsoft atenderá às solicitações de titulares de dados quanto a comentários coletados pela Microsoft (incluindo comentários gerenciados por subprocessadores da Microsoft), exceto em casos em que a Microsoft tenha instruído os usuários a não incluir dados pessoais durante o processo de coleta de comentários. Exceções: se a Microsoft tiver instruído os usuários a não incluir dados pessoais durante o processo de coleta de comentários, a Microsoft contará com essa instrução e presumirá que nenhum dado pessoal foi fornecido. Os usuários que criaram uma conta separada com provedores de serviços de comentários de terceiros devem atender ao DSR diretamente para esses provedores.
- Windows autenticado por meio de uma conta corporativa ou de estudante – se a sua organização tiver adquirido licenças do Windows e os usuários se autenticaram no Windows fornecido pela organização com a respectiva conta corporativa ou de estudante, a Microsoft atuará como um controlador de dados. Para saber mais, confira [Dados do Windows que o titular solicita para o RGPD](https://docs.microsoft.com/microsoft-365/compliance/gdpr-dsr-windows).
- Produtos ou serviços adquiridos pelo usuário – se você permitir que os usuários, agindo em sua capacidade individual, adquiram produtos ou serviços da Microsoft que usam o AAD para autenticação, por exemplo, Complementos do Office ou aplicativos disponíveis em uma Microsoft Store, a Microsoft poderá ser um controlador de dados. Para quaisquer um desses produtos ou serviços da Microsoft, os usuários precisarão contatar a Microsoft diretamente para iniciar uma DSR.
...
Se os usuários tiverem usado contas da Microsoft (isto é, contas pessoais) para adquirir produtos e serviços da Microsoft para próprio uso e para os quais a Microsoft é um controlador de dados, eles poderão iniciar solicitações de DSR usando o [painel de privacidade da Microsoft](https://account.microsoft.com/account/privacy).
...
- Usar a ferramenta de ocorrência de Descoberta Eletrônica da DSR no Centro de Conformidade e Segurança do Office 365 para gerenciar as investigações de DSR
- Configurar os Limites de Conformidade para limitar o escopo das Pesquisas de Conteúdo
- Usar a ferramenta de pesquisa do log de auditoria do Office 365 nas investigações de DSR
...
- Usar a pesquisa interna para pesquisar todo o conteúdo relacionado a um titular de dados específico. Quando você cria uma ocorrência e inicia a pesquisa, estes locais de conteúdo são pesquisados:
    - Todas as caixas de correio em sua organização (incluindo caixas de correio associadas em todos os Grupos do Office 365 e Microsoft Teams)
    - Todos os sites do SharePoint Online e contas do OneDrive for Business em sua organização
    - Todos os sites do Microsoft Teams e sites do grupo do Office 365 em sua organização
...
- Controlar quem tem acesso à ocorrência adicionando pessoas como membros da ocorrência; somente membros podem acessar a ocorrência e eles poderão ver apenas as respectivas ocorrências na lista de ocorrências na página de ocorrências de DSR no Centro de Conformidade e Segurança. Além disso, você pode atribuir diferentes permissões a diferentes membros da mesma ocorrência. Por exemplo, você pode permitir que alguns membros exibam somente a ocorrência e os resultados de uma Pesquisa de Conteúdo e permitir que outros membros criem pesquisas e exportem resultados da pesquisa.
...
- Criar trabalhos de exportação para exportar os resultados da pesquisa em resposta a uma solicitação de exportação de DSR. É possível exportar todo o conteúdo retornado pela Pesquisa de Conteúdo. Além disso, você pode exportar os logs gerados pelo sistema para o My Analytics e o serviço de Roaming do Office.
- Excluir ocorrências quando o processo de investigação de DSR é concluído. Isso removerá todas as pesquisas de conteúdo e exportará trabalhos associados à ocorrência.
...
Veja a seguir uma visão geral de alto nível de como implementar os Limites de Conformidade (com as ocorrências de Descoberta Eletrônica) para investigações de DSR.
...
5.  Adicione membros a cada um dos grupos de funções específicos como Gerentes de Descoberta Eletrônica. Os membros serão as pessoas responsáveis por investigar e responder às DSRs, e geralmente incluem administradores de TI, responsáveis pela privacidade de dados, gerentes de conformidade e representantes de recursos humanos.
...
Para obter instruções passo a passo, confira [Configurar limites de conformidade para investigações de Descoberta Eletrônica no Office 365](https://support.office.com/article/1b45c82f-26c8-44fb-9f3b-b45436fe2271).

### <a name="use-the-office-365-audit-log-search-tool-in-dsr-investigations"></a>Usar a ferramenta de pesquisa do log de auditoria do Office 365 nas investigações de DSR
...
É possível pesquisar o log de auditoria em busca de atividades específicas, tipos de atividade, atividades executadas por um usuário específico e outros critérios de pesquisa. Além das atividades do SharePoint Online e OneDrive for Business, você também pode pesquisar atividades no Flow, Power BI e Microsoft Teams. Observe que os registros de auditoria são retidos por 90 dias. Portanto, não será possível pesquisar as atividades do usuário que ocorreram há mais de 90 dias. Para obter uma lista completa das atividades auditadas e como pesquisar o log de auditoria, confira [Pesquisar o log de auditoria no Centro de Conformidade e Segurança do Office 365](https://support.office.com/article/Search-the-audit-log-in-the-Office-365-Security-Compliance-Center-0d4d0f35-390b-4518-800e-0c7ec95e946c).
