...
Além de implementação neste guia, certifique-se de configurar sites do SharePoint com a quantidade correta de proteção, inclusive garantindo permissões para conteúdo confidencial e altamente regulamentado são adequadas. Para obter mais informações sobre como criar sites para proteção de linha de base, confidenciais e altamente regulamentado, consulte [arquivos e sites seguro do SharePoint Online](https://docs.microsoft.com/office365/enterprise/secure-sharepoint-online-sites-and-files). 
...
O diagrama a seguir ilustra o conjunto de diretivas recomendadas para proteger os arquivos no SharePoint Online e o OneDrive for Business. Indica quais políticas serão atualizadas ou recém-criadas para adicionar a proteção para o SharePoint Online e o OneDrive for Business.

![Resumo das políticas para o SharePoint Online e OneDrive](../images/identity-access-ruleset-sharepoint.png)

Se você incluiu o SharePoint Online quando você criou as políticas comuns, você precisará criar os novos polcies. Ao configurar regras de acesso condicional, SharePoint Online inclui OneDrive for Business.
...
As novas políticas implementam a proteção de dispositivo do conteúdo confidencial e altamente regulamentado aplicando requisitos específicos de acesso aos sites do SharePoint que você especificar. 
 ...
 A tabela a seguir lista as políticas que é preciso examinar e atualizar ou criar novos para o SharePoint Online. As políticas comuns link para as instruções de configuração associada no artigo [comuns identity e dispositivo políticas de acesso](identity-access-policies.md) (links em breve).


|Nível de Proteção|Diretivas|Mais informações|
|:---------------|:-------|:----------------|
|**Linha de base**|[Exige MFA após a entrada risco *média* ou *alta*](identity-access-policies.md#require-mfa-based-on-sign-in-risk)|Inclua o SharePoint Online nas atribuições de aplicativos de nuvem.|
|        |[Bloquear os clientes que não oferecem suporte a autenticação moderna](identity-access-policies.md#block-clients-that-dont-support-modern-authentication)|Inclua o SharePoint Online nas atribuições de aplicativos de nuvem.|
...
|**Confidencial**|[Exige MFA após a entrada risco *baixa*, *média* ou *alta*](identity-access-policies.md#require-mfa-based-on-sign-in-risk)| Inclua o SharePoint Online nas atribuições de aplicativos de nuvem.|
|         |[Exigir compatível com PCs *e* dispositivos móveis](identity-access-policies.md#require-compliant-pcs-and-mobile-devices)|Inclua o SharePoint Online na lista de aplicativos na nuvem.|
||[Diretiva de controle de acesso do SharePoint Online](#sharepoint-online-access-control-policies): permitir acesso somente navegador a sites do SharePoint específicos de dispositivos não gerenciados|Isso impede que a edição e o download de arquivos. PowerShell do usuário para especificar os sites.|
|**Altamente controlada**|[*Sempre* requrie MFA](identity-access-policies.md#require-mfa-based-on-sign-in-risk)|Inclua o SharePoint Online nas atribuições de aplicativos de nuvem. |
||[Diretiva de controle de acesso do SharePoint Online](#use-app-enforced-restrictions-in-sharepoint-online): bloquear o acesso a sites do SharePoint específicos de dispositivos não gerenciados|Use o PowerShell para especificar os sites.|

## <a name="use-app-enforced-restrictions-in-sharepoint-online"></a>App imposta restrições de uso no SharePoint Online
...
- Sites confidenciais — permita acesso somente de navegador. Isso impede que usuários edição e baixar arquivos.
...
Na ilustração:
- James é atribuído às políticas de acesso condicional associadas à proteção de linha de base, mas ele pode ser oferecido acesso aos sites do SharePoint associados à proteção sigilosas ou altamente regulamentada. 
