...
---
# <a name="step-3-deploy-windows-10-enterprise-for-new-devices-with-windows-autopilot"></a><span data-ttu-id="9a25c-104">Etapa 3: Implantar o Windows 10 Enterprise para novos dispositivos com piloto automático do Windows</span><span class="sxs-lookup"><span data-stu-id="9a25c-104">Step 3: Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>

<span data-ttu-id="9a25c-105">*Este artigo se aplica às versões E3 tanto o E5 do Microsoft 365 Enterprise*</span><span class="sxs-lookup"><span data-stu-id="9a25c-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="9a25c-p101">Se você tiver novos Windows 10 PCs, você pode usar o piloto automático do Windows para personalizar fora da caixa-experiência (OOBE) para sua organização e implantar um novo sistema com aplicativos e configurações já configuradas. Há nenhum imagens para implantar, não há drivers inserir e nenhuma infra-estrutura para gerenciar. Os usuários possam ser efetuadas o processo de implantação de forma independente, sem a necessidade de consultar o seu administrador de TI.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p101">If you have new Windows 10 PCs, you can use Windows Autopilot to customize the out-of-box-experience (OOBE) for your organization and deploy a new system with apps and settings already configured. There are no images to deploy, no drivers to inject, and no infrastructure to manage. Users can go through the deployment process independently, without the need consult their IT administrator.</span></span>

<span data-ttu-id="9a25c-p102">Você pode configurar e pré-configurar novos dispositivos Windows 10 e conectá-los pronta para uso produtivo usando piloto automático do Windows. Para saber mais sobre o piloto automático do Windows, incluindo os benefícios e cenários de piloto automático do Windows, consulte [Visão geral do Windows piloto automático](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). Quando estiver pronto, siga estas partes para iniciar a configuração de novos dispositivos.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p102">You can set up and pre-configure new Windows 10 devices and get them ready for productive use using Windows Autopilot. To learn more about Windows Autopilot, including benefits and Windows Autopilot scenarios, see [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot). When ready, follow these parts to start setting up new devices.</span></span>

## <a name="part-1-start-windows-autopilot-deployment"></a><span data-ttu-id="9a25c-112">Parte 1: Implantação do piloto automático do Windows iniciar</span><span class="sxs-lookup"><span data-stu-id="9a25c-112">Part 1: Start Windows Autopilot deployment</span></span>
<span data-ttu-id="9a25c-113">Consulte [Visão geral do piloto automático do Windows](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) para:</span><span class="sxs-lookup"><span data-stu-id="9a25c-113">See [Overview of Windows Autopilot](https://docs.microsoft.com/windows/deployment/windows-Autopilot/windows-10-Autopilot) to:</span></span>

1. <span data-ttu-id="9a25c-p103">Saiba mais sobre e concluir os pré-requisitos para implantação do piloto automático do Windows. Os pré-requisitos incluem:</span><span class="sxs-lookup"><span data-stu-id="9a25c-p103">Learn about and complete the prerequisites for Windows Autopilot deployment. The prerequisites include:</span></span>
    - <span data-ttu-id="9a25c-116">**Registro do dispositivo e personalização da OOBE**</span><span class="sxs-lookup"><span data-stu-id="9a25c-116">**Device registration and OOBE customization**</span></span>

        <span data-ttu-id="9a25c-p104">Para registrar os dispositivos, você precisará adquirir sua ID de hardware e registrá-la. Estamos ativamente trabalhando com vários fornecedores de hardware para habilitá-los fornecer as informações necessárias para você, ou carregá-la em seu nome. Você também tem a opção para capturar essas informações por si mesmo usando um script do PowerShell que gera um arquivo. csv com ID de hardware. do dispositivo</span><span class="sxs-lookup"><span data-stu-id="9a25c-p104">To register devices, you need to acquire their hardware ID and register it. We are actively working with various hardware vendors to enable them to provide the required information to you, or upload it on your behalf. You also have the option to capture this information by yourself using a PowerShell script that generates a .csv file with the device's hardware ID.</span></span>

        <span data-ttu-id="9a25c-120">Depois que os dispositivos forem registrados, há opções de personalização de OOBE que você pode configurar incluindo ignorando o EULA e configurações de privacidade.</span><span class="sxs-lookup"><span data-stu-id="9a25c-120">Once devices are registered, there are OOBE customization options that you can configure including skipping privacy settings and EULA.</span></span>

    - <span data-ttu-id="9a25c-121">**Identidade visual da empresa para OOBE**</span><span class="sxs-lookup"><span data-stu-id="9a25c-121">**Company branding for OOBE**</span></span>

        <span data-ttu-id="9a25c-122">Isso permite que você adicione identidade visual para que ele apareça durante OOBE de dispositivo.</span><span class="sxs-lookup"><span data-stu-id="9a25c-122">This allows you to add branding to appear during device OOBE.</span></span>

    - <span data-ttu-id="9a25c-123">**Registro automático do MDM do Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="9a25c-123">**MDM auto-enrollment in Microsoft Intune**</span></span>
        
        <span data-ttu-id="9a25c-p105">A inscrição automática permite que os usuários registrar seus dispositivos Windows 10 no Intune para gerenciamento de dispositivo, quando eles se conectam seus dispositivos para o Windows Azure AD. Para registrar, os usuários adicionar sua conta do trabalho ao seus dispositivos pertencentes pessoalmente ou ingresse corporativo pertencentes dispositivos Azure AD. Em segundo plano, o dispositivo também está inscrito para o gerenciamento com Intune.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p105">Automatic enrollment lets users enroll their Windows 10 devices in Intune for device management when they connect their devices to Azure AD. To enroll, users add their work account to their personally-owned devices or join corporate-owned devices to Azure AD. In the background, the device is also enrolled for management with Intune.</span></span>

    - <span data-ttu-id="9a25c-127">**Conectividade de rede para os serviços de nuvem usados pelo Windows Autopilot**</span><span class="sxs-lookup"><span data-stu-id="9a25c-127">**Network connectivity to cloud services used by Windows Autopilot**</span></span>

        <span data-ttu-id="9a25c-128">Programa de implantação do piloto automático do Windows usa um número de serviços de nuvem para obter seus dispositivos para um estado produtivo e esses serviços devem ser acessíveis a partir de dispositivos registrados como os dispositivos de piloto automático do Windows.</span><span class="sxs-lookup"><span data-stu-id="9a25c-128">The Windows Autopilot Deployment Program uses a number of cloud services to get your devices to a productive state and these services must be accessible from devices registered as Windows Autopilot devices.</span></span> 

    - <span data-ttu-id="9a25c-129">**Os dispositivos devem estar pré-instalados no Windows 10, versão 1703 ou posterior**</span><span class="sxs-lookup"><span data-stu-id="9a25c-129">**Devices must be pre-installed with Windows 10, version 1703 or later**</span></span>

2. <span data-ttu-id="9a25c-p106">Saiba mais sobre e selecione o programa de implantação do piloto automático do Windows para sua organização. Você pode selecionar entre esses programas de implantação:</span><span class="sxs-lookup"><span data-stu-id="9a25c-p106">Learn about and select the Windows Autopilot Deployment Program for your organization. You can select from these deployment programs:</span></span>
    - <span data-ttu-id="9a25c-132">**Microsoft Store para Empresas**</span><span class="sxs-lookup"><span data-stu-id="9a25c-132">**Microsoft Store for Business**</span></span>
    - <span data-ttu-id="9a25c-133">**Microsoft Intune**</span><span class="sxs-lookup"><span data-stu-id="9a25c-133">**Microsoft Intune**</span></span>
    - <span data-ttu-id="9a25c-134">**Centro de parceiro**</span><span class="sxs-lookup"><span data-stu-id="9a25c-134">**Partner Center**</span></span>

## <a name="part-2-set-up-a-windows-10-device-for-microsoft-365"></a><span data-ttu-id="9a25c-135">Parte 2: Configurar um dispositivo Windows 10 para 365 da Microsoft</span><span class="sxs-lookup"><span data-stu-id="9a25c-135">Part 2: Set up a Windows 10 device for Microsoft 365</span></span>
<span data-ttu-id="9a25c-136">Antes de definir os dispositivos do Windows para usuários do Microsoft 365, verifique se todos os dispositivos do Windows estão executando o Windows 10, versão 1703 (atualização de criadores) ou posterior.</span><span class="sxs-lookup"><span data-stu-id="9a25c-136">Before you can set up Windows devices for Microsoft 365 users, make sure all the Windows devices are running Windows 10, version 1703 (Creators Update) or later.</span></span>

<span data-ttu-id="9a25c-137">Depois que todos os dispositivos do Windows em sua organização ou foram atualizados para o Windows Update de criadores de 10 ou já estão executando o Windows Update de criadores de 10, você pode ingressar esses dispositivos da sua organização do Azure Active Directory.</span><span class="sxs-lookup"><span data-stu-id="9a25c-137">After all Windows devices in your organization have either been upgraded to Windows 10 Creators Update or are already running Windows 10 Creators Update, you can join these devices to your organization’s Azure Active Directory.</span></span>

### <a name="set-up-a-brand-new-or-newly-upgraded-windows-10-device"></a><span data-ttu-id="9a25c-138">Configurar um dispositivo do Windows 10 totalmente novo ou atualizados recentemente</span><span class="sxs-lookup"><span data-stu-id="9a25c-138">Set up a brand new or newly-upgraded Windows 10 device</span></span>
<span data-ttu-id="9a25c-139">Siga estas etapas para configurar um dispositivo usando o Windows 10 OOBE em um novo dispositivo executando o Windows Update de criadores de 10 (ou posterior) ou em um dispositivo que era a atualização para o Windows Update de criadores de 10 (ou posterior), mas não passou por imediata de instalação.</span><span class="sxs-lookup"><span data-stu-id="9a25c-139">Follow these steps to set up a device using the Windows 10 OOBE on a brand new device running Windows 10 Creators Update (or later) or on a device that was upgrade to Windows 10 Creators Update (or later) but has not gone through out-of-box setup.</span></span>

1. <span data-ttu-id="9a25c-140">Se você não tiver uma rede sem fio configurada, certifique-se de que conectar o dispositivo à internet por meio de um com fio ou conexão Ethernet.</span><span class="sxs-lookup"><span data-stu-id="9a25c-140">If you don't have a wireless network configured, make sure you connect the device to the internet through a wired or Ethernet connection.</span></span>
2. <span data-ttu-id="9a25c-p107">Passar a experiência de configuração de dispositivo do Windows. Em um dispositivo novo ou redefinido, a experiência de configuração começará com o **Vamos iniciar com a região. É esse direito?** tela.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p107">Go through the Windows device setup experience. On a new or reset device, the setup experience starts with the **Let's start with region. Is this right?** screen.</span></span>
3. <span data-ttu-id="9a25c-p108">Vá por meio da instalação de dispositivo do Windows 10 até chegar ao **como você deseja configurar?** página. Aqui, selecione **Configurar para uma organização**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p108">Go through Windows 10 device setup until you get to the **How would you like to set up?** page. Here, select **Set up for an organization**.</span></span>
4. <span data-ttu-id="9a25c-p109">Conectar usando a conta e a senha do usuário Microsoft 365. Dependendo da configuração de senha do usuário, você pode ser solicitado para atualizar a senha.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p109">Sign in using the Microsoft 365 user's account and password. Depending on the user password setting, you may be prompted to update the password.</span></span> 
5. <span data-ttu-id="9a25c-147">Conclua a configuração do dispositivo Windows 10.</span><span class="sxs-lookup"><span data-stu-id="9a25c-147">Finish Windows 10 device setup.</span></span>

<span data-ttu-id="9a25c-148">Depois de terminar, o dispositivo será conectado ao Azure da sua organização AD.</span><span class="sxs-lookup"><span data-stu-id="9a25c-148">After you’re done, the device will be connected to your organization’s Azure AD.</span></span>

### <a name="set-up-a-device-that-has-already-completed-out-of-box-setup"></a><span data-ttu-id="9a25c-149">Configurar um dispositivo que já tenha concluído a instalação do imediata de</span><span class="sxs-lookup"><span data-stu-id="9a25c-149">Set up a device that has already completed out-of-box setup</span></span>
<span data-ttu-id="9a25c-150">Se o dispositivo tiver o Windows Update de criadores de 10 (ou posterior) e tiver ido já por meio da configuração de caixa de limite, siga estas etapas.</span><span class="sxs-lookup"><span data-stu-id="9a25c-150">If your device has Windows 10 Creators Update (or later) and has already gone through the out-of-box setup, follow these steps.</span></span>

1. <span data-ttu-id="9a25c-151">Em PC com Windows do usuário que está executando o Windows 10, versão 1703 (atualização de criadores), selecione o logotipo do **Windows** e, em seguida, selecione o ícone de **configurações** .</span><span class="sxs-lookup"><span data-stu-id="9a25c-151">On your user's Windows PC that is running Windows 10, version 1703 (Creators Update), select the **Windows** logo, and then select the **Settings** icon.</span></span>
2. <span data-ttu-id="9a25c-152">Em **Configurações**, vá para **Contas**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-152">In **Settings**, go to **Accounts**.</span></span>
3. <span data-ttu-id="9a25c-153">Na página **seu info** , selecione **acesso de trabalho ou escola** > **Connect**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-153">On the **Your info** page, select **Access work or school** > **Connect**.</span></span>
4. <span data-ttu-id="9a25c-154">Na caixa de diálogo **Configurar uma conta de trabalho ou da escola** , em **ações alternativas**, selecione **ingressar este dispositivo para o Windows Azure Active Directory**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-154">On the **Set up a work or school account** dialog, under **Alternate actions**, select **Join this device to Azure Active Directory**.</span></span>
5. <span data-ttu-id="9a25c-155">Na página **vamos fazer com que você entrar** , insira seu trabalho ou escola conta e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-155">On the **Let's get you signed in** page, enter your work or school account, and select **Next**.</span></span>
6. <span data-ttu-id="9a25c-156">Na página **Digite a senha** , digite sua senha e selecione **entrar**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-156">On the **Enter password** page, enter your password, and select **Sign in**.</span></span>
7. <span data-ttu-id="9a25c-157">Na página **certificar-se de que esta é a sua organização** , verifique se as informações estão corretas e selecione **ingressar**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-157">On the **Make sure this is your organization** page, verify that the information is correct, and select **Join**.</span></span>
8. <span data-ttu-id="9a25c-p110">Sobre o **você estará pronto!** página, selecione **concluído**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p110">On the **You're all set!** page, select **Done**.</span></span>

<span data-ttu-id="9a25c-160">Depois de terminar, o usuário será conectado ao Azure da sua organização AD.</span><span class="sxs-lookup"><span data-stu-id="9a25c-160">After you're done, the user will be connected to your organization's Azure AD.</span></span>

### <a name="verify-the-device-is-connected-to-azure-ad"></a><span data-ttu-id="9a25c-161">Verifique se o dispositivo está conectado ao Azure AD</span><span class="sxs-lookup"><span data-stu-id="9a25c-161">Verify the device is connected to Azure AD</span></span>
<span data-ttu-id="9a25c-162">Siga estas etapas para verificar o status de sincronização do dispositivo com o Azure AD e inicie usando sua conta da Microsoft 365 no dispositivo.</span><span class="sxs-lookup"><span data-stu-id="9a25c-162">Follow these steps to verify the device’s sync status with Azure AD, and then start using your Microsoft 365 account on the device.</span></span> 

1. <span data-ttu-id="9a25c-163">Abra **configurações**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-163">Open **Settings**.</span></span>
2. <span data-ttu-id="9a25c-164">Na página **Access trabalho ou escola** , selecione o \*\*conectado à <organization name> \*\* área expor os botões **Info** e **Disconnect**.</span><span class="sxs-lookup"><span data-stu-id="9a25c-164">On the **Access work or school** page, select the **Connected to <organization name>** area to expose the buttons **Info** and **Disconnect**.</span></span>
3. <span data-ttu-id="9a25c-165">Selecione **Info** para obter seu status de sincronização.</span><span class="sxs-lookup"><span data-stu-id="9a25c-165">Select **Info** to get your synchronization status.</span></span>
4. <span data-ttu-id="9a25c-166">Na página **status da sincronização** , selecione **Sync** para obter o dispositivo móvel mais recente políticas de gerenciamento no PC.</span><span class="sxs-lookup"><span data-stu-id="9a25c-166">On the **Sync status** page, select **Sync** to get the latest mobile device management policies onto the PC.</span></span>
5. <span data-ttu-id="9a25c-167">Para começar a usar a conta da Microsoft 365, vá para o botão **Iniciar** do Windows, sua imagem de conta atual do mouse em e selecione conta de **comutador** .</span><span class="sxs-lookup"><span data-stu-id="9a25c-167">To start using the Microsoft 365 account, go to the Windows **Start** button, right-click your current account picture and then select **Switch** account.</span></span>
6. <span data-ttu-id="9a25c-168">Entre com o email e a senha da sua organização.</span><span class="sxs-lookup"><span data-stu-id="9a25c-168">Sign in by using your organization email and password.</span></span>

<span data-ttu-id="9a25c-p111">Se você tiver problemas ao usar o Windows 10 em um ambiente empresarial, você pode consultar as [principais soluções de suporte da Microsoft para os problemas mais comuns](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). Esses recursos incluem artigos KB, atualizações e artigos da biblioteca.</span><span class="sxs-lookup"><span data-stu-id="9a25c-p111">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="9a25c-171">Como ponto de verificação provisório, você pode consultar os [critérios de saída](windows10-exit-criteria.md#crit-windows10-step3) para esta etapa.</span><span class="sxs-lookup"><span data-stu-id="9a25c-171">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step3) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="9a25c-172">Próxima etapa</span><span class="sxs-lookup"><span data-stu-id="9a25c-172">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step4.png)| [<span data-ttu-id="9a25c-173">Análise do Windows usar para monitorar a integridade do dispositivo</span><span class="sxs-lookup"><span data-stu-id="9a25c-173">Use Windows Analytics to monitor device health</span></span>](windows10-enable-windows-analytics.md) |
