...
---
# <a name="step-2-deploy-windows-10-enterprise-for-existing-devices-as-an-in-place-upgrade"></a><span data-ttu-id="55878-104">Etapa 2: Implantar o Windows 10 Enterprise para dispositivos existentes como uma atualização in-loco</span><span class="sxs-lookup"><span data-stu-id="55878-104">Step 2: Deploy Windows 10 Enterprise for existing devices as an in-place upgrade</span></span>

<span data-ttu-id="55878-105">*Este artigo se aplica às versões E3 tanto o E5 do Microsoft 365 Enterprise*</span><span class="sxs-lookup"><span data-stu-id="55878-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="55878-p101">O caminho mais simples para atualizar PCs atualmente executando o Windows 7 ou Windows 8.1 para Windows 10 é por meio de uma atualização in-loco. Você pode usar uma sequência de tarefas do System Center Configuration Manager (Configuration Manager) para automatizar completamente o processo.</span><span class="sxs-lookup"><span data-stu-id="55878-p101">The simplest path to upgrade PCs currently running Windows 7 or Windows 8.1 to Windows 10 is through an in-place upgrade. You can use a System Center Configuration Manager (Configuration Manager) task sequence to completely automate the process.</span></span> 

<span data-ttu-id="55878-p102">Se você tiver computadores existentes que executam o Windows 7 ou Windows 8.1, recomendamos esse caminho se sua organização está implantando o Windows 10. Isso aproveita o programa de instalação do Windows (Setup.exe) para executar uma atualização in-loco, que automaticamente preserva todos os dados, configurações, aplicativos, e os drivers da versão de sistema operacional existente. Isso exige o mínimo de esforço IT, porque não há nenhuma necessidade de infraestrutura de qualquer implantação complexa.</span><span class="sxs-lookup"><span data-stu-id="55878-p102">If you have existing computers running Windows 7 or Windows 8.1, we recommend this path if your organization is deploying Windows 10. This leverages the Windows installation program (Setup.exe) to perform an in-place upgrade, which automatically preserves all data, settings, applications, and drivers from the existing operating system version. This requires the least IT effort, because there is no need for any complex deployment infrastructure.</span></span>

<span data-ttu-id="55878-111">Siga estas etapas para configurar e implantar uma imagem do Windows 10 Enterprise usando o Configuration Manager como uma atualização in-loco.</span><span class="sxs-lookup"><span data-stu-id="55878-111">Follow these steps to configure and deploy a Windows 10 Enterprise image using Configuration Manager as an in-place upgrade.</span></span>

## <a name="part-1-verify-readiness-to-upgrade-windows"></a><span data-ttu-id="55878-112">Parte 1: Verificar a preparação para atualizar o Windows</span><span class="sxs-lookup"><span data-stu-id="55878-112">Part 1: Verify readiness to upgrade Windows</span></span>

<span data-ttu-id="55878-p103">Primeiro, use a capacidade de preparação de atualização do Windows análise para fornecer ideias poderosas e recomendações sobre os computadores, aplicativos e drivers em sua organização, sem nenhum custo extra e sem requisitos de infraestrutura adicional. Esse novo serviço orienta você atualização e recurso atualizar projetos usando um fluxo de trabalho com base no Microsoft práticas recomendada. Dados de inventário atualizado permite custos de equilíbrio e os riscos em seus projetos de atualização.</span><span class="sxs-lookup"><span data-stu-id="55878-p103">First, use the Upgrade Readiness capability of Windows Analytics to provide powerful insights and recommendations about the computers, applications, and drivers in your organization, at no extra cost and without additional infrastructure requirements. This new service guides you through upgrade and feature update projects using a workflow based on Microsoft recommended practices. Up-to-date inventory data allows you to balance cost and risk in your upgrade projects.</span></span>

<span data-ttu-id="55878-116">Consulte [as atualizações de gerenciar o Windows com atualização preparação](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) para saber mais, introdução, use e solucionar problemas de preparação de atualização.</span><span class="sxs-lookup"><span data-stu-id="55878-116">See [Manage Windows upgrades with Upgrade Readiness](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) to learn more, get started, use, and troubleshoot Upgrade Readiness.</span></span>

<span data-ttu-id="55878-p104">Em seguida, siga o guia para usar o System Center Configuration Manager (ramificação atual) para atualizar o Windows 7 ou o sistema operacional posterior para Windows 10. Assim como acontece com qualquer implantação de alto risco, é recomendável fazer backup dos dados do usuário antes de continuar. Armazenamento em nuvem OneDrive está pronto para uso para os usuários licenciados do Microsoft 365 e pode ser usado para armazenar com segurança seus arquivos. Para obter mais informações, consulte o [guia de início rápido do OneDrive](https://aka.ms/ODfBquickstartguide). Para acessar esta página, você deve entrar no como um administrador proprietário ou administrador global em um locatário do Office 365 ou Microsoft 365.</span><span class="sxs-lookup"><span data-stu-id="55878-p104">Next, follow the guide to use System Center Configuration Manager (Current Branch) to upgrade Windows 7 or later operating system to Windows 10. As with any high-risk deployment, we recommend backing up user data before proceeding. OneDrive cloud storage is ready to use for licensed Microsoft 365 users and can be used to securely store their files. For more info, see [OneDrive quick start guide](https://aka.ms/ODfBquickstartguide). To access this page, you must sign in as a tenant admin or global admin in an Office 365 or Microsoft 365 tenant.</span></span>

<span data-ttu-id="55878-122">Para obter uma lista de versões do Configuration Manager e as versões do cliente Windows 10 correspondentes suportados, consulte [suporte para Windows 10 para o System Center Configuration Manager](https://aka.ms/supportforwin10sccm).</span><span class="sxs-lookup"><span data-stu-id="55878-122">For a list of Configuration Manager versions and the corresponding Windows 10 client versions that are supported, see [Support for Windows 10 for System Center Configuration Manager](https://aka.ms/supportforwin10sccm).</span></span>

<span data-ttu-id="55878-123">**Para verificar a preparação para atualizar o Windows**</span><span class="sxs-lookup"><span data-stu-id="55878-123">**To verify readiness to upgrade Windows**</span></span>

<span data-ttu-id="55878-124">Examine esses requisitos antes de começar sua implantação do Windows 10:</span><span class="sxs-lookup"><span data-stu-id="55878-124">Review these requirements before starting your Windows 10 deployment:</span></span>

- <span data-ttu-id="55878-p105">**Edições de Windows elegíveis para atualizar** - seus dispositivos devem estar executando edições do Windows 7 ou Windows 8.1 elegíveis para a atualização para o Windows 10 Enterprise. Para obter uma lista de edições suportadas, consulte [caminhos de atualização do Windows 10](https://aka.ms/win10upgradepaths).</span><span class="sxs-lookup"><span data-stu-id="55878-p105">**Windows editions eligible for upgrade** - Your devices must be running editions of Windows 7 or Windows 8.1 that are eligible for upgrade to Windows 10 Enterprise. For a list of supported editions, see [Windows 10 upgrade paths](https://aka.ms/win10upgradepaths).</span></span> 
- <span data-ttu-id="55878-p106">**Dispositivos suportados** - a maioria dos computadores que são compatíveis com o Windows 8.1 será compatível com o Windows 10. Talvez seja necessário instalar os drivers atualizados no Windows 10 para os seus dispositivos funcione corretamente. Consulte [as especificações do Windows 10](https://aka.ms/windows10specifications) para obter mais informações.</span><span class="sxs-lookup"><span data-stu-id="55878-p106">**Supported devices** - Most computers that are compatible with Windows 8.1 will be compatible with Windows 10. You may need to install updated drivers in Windows 10 for your devices to properly function. See [Windows 10 specifications](https://aka.ms/windows10specifications) for more info.</span></span>
- <span data-ttu-id="55878-130">**Preparação para implantação** - Verifique se você tem o seguinte antes de começar a configurar a implantação:</span><span class="sxs-lookup"><span data-stu-id="55878-130">**Deployment preparation** - Make sure you have the following before you start configuring the deployment:</span></span>
    - <span data-ttu-id="55878-p107">Mídia de instalação do Windows 10 - da mídia de instalação deve estar localizada em uma unidade separada, com o ISO já montado. Você pode obter o ISO a partir de [Downloads para assinantes do MSDN](https://aka.ms/msdn-subscriber-downloads) ou do [Centro de atendimento de licenciamento por Volume](https://aka.ms/mvlsc).</span><span class="sxs-lookup"><span data-stu-id="55878-p107">Windows 10 installation media - The installation media must be located on a separate drive, with the ISO already mounted. You can obtain the ISO from [MSDN Subscriber Downloads](https://aka.ms/msdn-subscriber-downloads) or from the [Volume Licensing Service Center](https://aka.ms/mvlsc).</span></span>
    - <span data-ttu-id="55878-p108">Backups de dados de usuário - embora os dados de usuário serão migrados na atualização, prática recomendada é configurar um cenário de backup. Por exemplo, exporte todos os dados de usuário para uma conta de OneDrive, unidade flash USB criptografadas BitLocker To Go ou servidor de arquivos de rede. Para obter mais informações, consulte [backup ou transferir dados no Windows](https://aka.ms/backuptransferdatawindows).</span><span class="sxs-lookup"><span data-stu-id="55878-p108">Backups of user data - Although user data will be migrated in the upgrade, best practice is to configure a backup scenario. For example, export all user data to a OneDrive account, BitLocker To Go-encrypted USB flash drive, or network file server. For more information, see [Back up or transfer data in Windows](https://aka.ms/backuptransferdatawindows).</span></span>
- <span data-ttu-id="55878-p109">**Preparação do ambiente** - você usará uma estrutura de servidor existente do Configuration Manager para preparar a implantação do sistema operacional. Além da configuração básica, as configurações a seguir devem ser feitas no ambiente do Configuration Manager:</span><span class="sxs-lookup"><span data-stu-id="55878-p109">**Environment preparation** - You will use an existing Configuration Manager server structure to prepare for operating system deployment. In addition to the base setup, the following configurations should be made in the Configuration Manager environment:</span></span>
    1. <span data-ttu-id="55878-138">[Estender o esquema do Active Directory](https://aka.ms/extendadschema) e [Crie um contêiner de gerenciamento do sistema](https://aka.ms/createsysmancontainer).</span><span class="sxs-lookup"><span data-stu-id="55878-138">[Extend the Active Directory Schema](https://aka.ms/extendadschema) and [create a System Management container](https://aka.ms/createsysmancontainer).</span></span>
    2. <span data-ttu-id="55878-p110">Descoberta de habilitar floresta do Active Directory e descoberta de sistema do Active Directory. Para obter mais informações, consulte [Configure métodos de descoberta para o System Center Configuration Manager](https://aka.ms/configurediscoverymethods).</span><span class="sxs-lookup"><span data-stu-id="55878-p110">Enable Active Directory Forest Discovery and Active Directory System Discovery. For more info, see [Configure discovery methods for System Center Configuration Manager](https://aka.ms/configurediscoverymethods).</span></span>
    3. <span data-ttu-id="55878-p111">Crie limites de intervalo IP e o grupo de limite para atribuição de conteúdo e sites. Para obter mais informações, consulte [definir limites de site e de grupos de limite para o System Center Configuration Manager](https://aka.ms/definesiteboundaries).</span><span class="sxs-lookup"><span data-stu-id="55878-p111">Create IP range boundaries and boundary group for content and site assignment. For more info, see [Define site boundaries and boundary groups for System Center Configuration Manager](https://aka.ms/definesiteboundaries).</span></span>
    4. <span data-ttu-id="55878-p112">Adicionar e configurar o Gerenciador de configuração do reporting services ponto de função. Para obter mais informações, consulte [Configurando relatórios no Gerenciador de configuração](https://aka.ms/configurereporting).</span><span class="sxs-lookup"><span data-stu-id="55878-p112">Add and configure the Configuration Manager reporting services point role. For more info, see [Configuring Reporting in Configuration Manager](https://aka.ms/configurereporting).</span></span>
    5. <span data-ttu-id="55878-145">Crie uma estrutura de pastas do sistema de arquivo para os pacotes.</span><span class="sxs-lookup"><span data-stu-id="55878-145">Create a file system folder structure for packages.</span></span>
    6. <span data-ttu-id="55878-146">Crie uma estrutura de pastas de console do Configuration Manager para os pacotes.</span><span class="sxs-lookup"><span data-stu-id="55878-146">Create a Configuration Manager console folder structure for packages.</span></span>
    7. <span data-ttu-id="55878-147">Instale atualizações do System Center Configuration Manager (ramificação atual) e qualquer pré-requisitos adicionais do Windows 10.</span><span class="sxs-lookup"><span data-stu-id="55878-147">Install System Center Configuration Manager (Current Branch) updates and any additional Windows 10 prerequisites.</span></span>

## <a name="part-2-add-a-windows-10-os-image-using-configuration-manager"></a><span data-ttu-id="55878-148">Parte 2: Adicionar uma imagem de sistema operacional do Windows 10 usando o Configuration Manager</span><span class="sxs-lookup"><span data-stu-id="55878-148">Part 2: Add a Windows 10 OS image using Configuration Manager</span></span>
<span data-ttu-id="55878-p113">Agora, você precisará criar um pacote de atualização do sistema operacional que contém a mídia de instalação completa do Windows 10. As etapas a seguir, você usará o Configuration Manager para criar um pacote de atualização para Windows 10 Enterprise x64.</span><span class="sxs-lookup"><span data-stu-id="55878-p113">Now you'll need to create an operating system upgrade package that contains the full Windows 10 installation media. In the following steps, you’ll use Configuration Manager to create an upgrade package for Windows 10 Enterprise x64.</span></span>

<span data-ttu-id="55878-151">**Para adicionar uma imagem de sistema operacional do Windows 10 usando o Configuration Manager**</span><span class="sxs-lookup"><span data-stu-id="55878-151">**To add a Windows 10 OS image using Configuration Manager**</span></span>

1. <span data-ttu-id="55878-152">Usando o console do Configuration Manager, no espaço de trabalho de **Biblioteca de Software** , com o botão direito no nó **Pacotes de atualização de sistema operacional** e selecione **Adicionar pacote de atualização do sistema operacional**.</span><span class="sxs-lookup"><span data-stu-id="55878-152">Using the Configuration Manager console, in the **Software Library** workspace, right-click the **Operating System Upgrade Packages** node, and then select **Add Operating System Upgrade Package**.</span></span>
2. <span data-ttu-id="55878-153">Na página de **Fonte de dados** , especifique o caminho UNC para a mídia do Windows 10 Enterprise x64 e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-153">On the **Data Source** page, specify the UNC path to the Windows 10 Enterprise x64 media, and then select **Next**.</span></span>
3. <span data-ttu-id="55878-154">Na página **Geral** , especifique a **atualização do Windows 10 Enterprise x64**e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-154">On the **General** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span> 
4. <span data-ttu-id="55878-155">Na página **Resumo** , selecione **Avançar**e, em seguida, selecione **Fechar**.</span><span class="sxs-lookup"><span data-stu-id="55878-155">On the **Summary** page, select **Next**, and then select **Close**.</span></span> 
5. <span data-ttu-id="55878-156">O pacote de **atualização do Windows 10 Enterprise x64** criado do mouse em e selecione **Distribuir conteúdo**.</span><span class="sxs-lookup"><span data-stu-id="55878-156">Right-click the created **Windows 10 Enterprise x64 Update** package, and then select **Distribute Content**.</span></span> 
6. <span data-ttu-id="55878-157">Escolha seu ponto de distribuição.</span><span class="sxs-lookup"><span data-stu-id="55878-157">Choose your distribution point.</span></span>

## <a name="part-3-configure-deployment-settings"></a><span data-ttu-id="55878-158">Parte 3: Configurar definições de implantação</span><span class="sxs-lookup"><span data-stu-id="55878-158">Part 3: Configure deployment settings</span></span>
<span data-ttu-id="55878-p114">Nesta etapa, você vai configurar uma sequência de atualização de tarefa que contém as configurações para a atualização do Windows 10. Você, em seguida, identifique a atualização dos dispositivos e então implantar a sequência de tarefas para esses dispositivos.</span><span class="sxs-lookup"><span data-stu-id="55878-p114">In this step, you'll configure an upgrade task sequence that contains the settings for the Windows 10 upgrade. You'll then identify the devices to upgrade, and then deploy the task sequence to those devices.</span></span>

### <a name="create-a-task-sequence"></a><span data-ttu-id="55878-161">Criar uma sequência de tarefas</span><span class="sxs-lookup"><span data-stu-id="55878-161">Create a task sequence</span></span>
<span data-ttu-id="55878-162">Para criar uma sequência de atualização de tarefa, execute as seguintes etapas:</span><span class="sxs-lookup"><span data-stu-id="55878-162">To create an upgrade task sequence, perform the following steps:</span></span>
  
1. <span data-ttu-id="55878-163">No console do Configuration Manager, no espaço de trabalho de **Biblioteca de Software** , expanda **sistemas operacionais**.</span><span class="sxs-lookup"><span data-stu-id="55878-163">In the Configuration Manager console, in the **Software Library** workspace, expand **Operating Systems**.</span></span> 
2. <span data-ttu-id="55878-164">Com o botão direito no nó de **Sequências de tarefas** e, em seguida, selecione **Criar sequência de tarefa**.</span><span class="sxs-lookup"><span data-stu-id="55878-164">Right-click the **Task Sequences** node, and then select **Create Task Sequence**.</span></span>
3. <span data-ttu-id="55878-165">Na página **criar uma nova sequência de tarefas** , selecione **atualizar um sistema operacional do pacote de atualização**e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-165">On the **Create a new task sequence** page, select **Upgrade an operating system from upgrade package**, and then select **Next**.</span></span>
4. <span data-ttu-id="55878-166">Na página **Informações de sequência de tarefas** , especifique a **atualização do Windows 10 Enterprise x64**e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-166">On the **Task Sequence Information** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span>
5. <span data-ttu-id="55878-167">Na página **atualizar o sistema operacional Windows** , selecione **navegar** e escolha o **pacote de atualização do sistema operacional da atualização Windows 10 Enterprise x64**, selecione **Okey**e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-167">On the **Upgrade the Windows operating system** page, select **Browse** and choose the **Windows 10 Enterprise x64 Upgrade operating system upgrade package**, select **OK**, and then select **Next**.</span></span>
6. <span data-ttu-id="55878-168">Continue páginas restantes do assistente e, em seguida, selecione **Fechar**.</span><span class="sxs-lookup"><span data-stu-id="55878-168">Continue through the remaining wizard pages, and then select **Close**.</span></span>

### <a name="create-a-device-collection"></a><span data-ttu-id="55878-169">Criar um conjunto de dispositivo</span><span class="sxs-lookup"><span data-stu-id="55878-169">Create a device collection</span></span>
<span data-ttu-id="55878-170">Depois de criar a sequência de atualização de tarefa, você precisará criar uma coleção que contém os dispositivos que você as atualizará.</span><span class="sxs-lookup"><span data-stu-id="55878-170">After you create the upgrade task sequence, you'll need to create a collection that contains the devices you will upgrade.</span></span>

> [!NOTE]
> <span data-ttu-id="55878-p115">Use as configurações a seguir para testar a implantação em um único dispositivo. Você pode usar as regras de associação diferentes para incluir grupos de dispositivos quando estiver pronto. Para obter mais informações, consulte [como criar conjuntos no System Center Configuration Manager](https://aka.ms/sccm-create-collections).</span><span class="sxs-lookup"><span data-stu-id="55878-p115">Use the following settings to test the deployment on a single device. You can use different membership rules to include groups of devices when you are ready. For more info, see [How to create collections in System Center Configuration Manager](https://aka.ms/sccm-create-collections).</span></span>

1. <span data-ttu-id="55878-174">No console do Configuration Manager, no espaço de trabalho **ativos e conformidade** , clique com o botão **Coleções de dispositivo**e selecione **Criar conjunto de dispositivo**.</span><span class="sxs-lookup"><span data-stu-id="55878-174">In the Configuration Manager console, in the **Assets and Compliance** workspace, right-click **Device Collections**, and then select **Create Device Collection**.</span></span> 
2. <span data-ttu-id="55878-175">No assistente criar conjunto de dispositivo, na página **Geral** , insira as seguintes configurações e selecione **Avançar**:</span><span class="sxs-lookup"><span data-stu-id="55878-175">In the Create Device Collection wizard, on the **General** page, enter the following settings and then select **Next**:</span></span>
    - <span data-ttu-id="55878-176">Nome: Windows 10 Enterprise x64 atualização</span><span class="sxs-lookup"><span data-stu-id="55878-176">Name: Windows 10 Enterprise x64 Upgrade</span></span>
    - <span data-ttu-id="55878-177">Coleção limitante: Todos os sistemas</span><span class="sxs-lookup"><span data-stu-id="55878-177">Limiting Collection: All Systems</span></span>
3. <span data-ttu-id="55878-178">Na página **Regras de participação** , selecione **Adicionar regra** > **regra direta** para iniciar o Assistente para criar regra de associação direta.</span><span class="sxs-lookup"><span data-stu-id="55878-178">On the **Membership Rules** page, select **Add Rule** > **Direct rule** to launch the Create Direct Membership Rule Wizard.</span></span>
4. <span data-ttu-id="55878-179">Na página de **boas-vindas** do Assistente para criar regra de associação direta, selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-179">On the **Welcome** page of the Create Direct Membership Rule Wizard, select **Next**.</span></span>
5. <span data-ttu-id="55878-180">Na página de **pesquisa de recursos** , insira as configurações a seguir, substituindo o **valor** de texto de espaço reservado com o nome do dispositivo que você está atualizando:</span><span class="sxs-lookup"><span data-stu-id="55878-180">On the **Search for Resources** page, enter the following settings, replacing the placeholder **Value** text with the name of the device you are upgrading:</span></span> 
    - <span data-ttu-id="55878-181">Classe de recursos: Recursos do sistema</span><span class="sxs-lookup"><span data-stu-id="55878-181">Resource Class: System Resource</span></span>
    - <span data-ttu-id="55878-182">Nome do atributo: nome</span><span class="sxs-lookup"><span data-stu-id="55878-182">Attribute Name: Name</span></span>
    - <span data-ttu-id="55878-183">Valor: *PC0003*</span><span class="sxs-lookup"><span data-stu-id="55878-183">Value: *PC0003*</span></span>
6. <span data-ttu-id="55878-184">Na página **Selecionar recursos** , selecione seu dispositivo e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-184">On the **Select Resources** page, select your device, and select **Next**.</span></span>
7. <span data-ttu-id="55878-185">Conclua o Assistente para criar a regra de associação direta e o Assistente para criação de conjunto de dispositivo.</span><span class="sxs-lookup"><span data-stu-id="55878-185">Complete the Create Direct Membership Rule wizard and the Create Device Collection Wizard.</span></span>  
8. <span data-ttu-id="55878-p116">Examine a coleção de atualização do Windows 10 Enterprise x64. Não continue até que você veja as máquinas adicionado na coleção.</span><span class="sxs-lookup"><span data-stu-id="55878-p116">Review the Windows 10 Enterprise x64 Upgrade collection. Do not continue until you see the machines you added in the collection.</span></span>

### <a name="create-an-operating-system-deployment"></a><span data-ttu-id="55878-188">Criar uma implantação do sistema operacional</span><span class="sxs-lookup"><span data-stu-id="55878-188">Create an operating system deployment</span></span>
<span data-ttu-id="55878-189">Siga estas etapas para criar uma implantação da sequência de tarefas.</span><span class="sxs-lookup"><span data-stu-id="55878-189">Follow these steps to create a deployment for the task sequence.</span></span>

1. <span data-ttu-id="55878-190">No console do Configuration Manager, no espaço de trabalho **Software biblioteca** , clique com o botão a sequência de tarefas que você criou na etapa anterior e selecione **Deploy**.</span><span class="sxs-lookup"><span data-stu-id="55878-190">In the Configuration Manager console, in the **Software Library** workspace, right-click the task sequence you created in a previous step, and then select **Deploy**.</span></span>
2. <span data-ttu-id="55878-191">Na página **Geral** , selecione o conjunto de **atualização do Windows 10 Enterprise x64** e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-191">On the **General** page, select the **Windows 10 Enterprise x64 Upgrade** collection, and then select **Next**.</span></span>
3. <span data-ttu-id="55878-192">Na página de **conteúdo** , selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-192">On the **Content** page, select **Next**.</span></span>
4. <span data-ttu-id="55878-193">Na página **Definições de implantação** , selecione as seguintes configurações e selecione **Avançar**:</span><span class="sxs-lookup"><span data-stu-id="55878-193">On the **Deployment Settings** page, select the following settings, and then select **Next**:</span></span>

    > [!NOTE]
    > <span data-ttu-id="55878-p117">Para esta implantação de teste, você definirá a finalidade como **disponível**, que requer intervenção do usuário para iniciar a implantação. Em um ambiente de produção, talvez você queira automatizar a implantação usando a finalidade necessária, o que envolve a configuração opções adicionais, como agendar quando a implantação é executada.</span><span class="sxs-lookup"><span data-stu-id="55878-p117">For this test deployment, you'll set the purpose to **Available**, which requires user intervention to start the deployment. In a production environment, you may wish to automate the deployment using the Required purpose, which involves configuring additional options such as scheduling when the deployment is run.</span></span> 

    - <span data-ttu-id="55878-196">Ação: instalar</span><span class="sxs-lookup"><span data-stu-id="55878-196">Action: Install</span></span>
    - <span data-ttu-id="55878-197">Objetivo: disponíveis</span><span class="sxs-lookup"><span data-stu-id="55878-197">Purpose: Available</span></span>

5. <span data-ttu-id="55878-198">Na página **agendamento** , aceite as configurações padrão e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-198">On the **Scheduling** page, accept the default settings, and then select **Next**.</span></span>
6. <span data-ttu-id="55878-199">Na página de **Experiência do usuário** , aceite as configurações padrão e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-199">On the **User Experience** page, accept the default settings, and then select **Next**.</span></span>
7. <span data-ttu-id="55878-200">Na página **alertas** , aceite as configurações padrão e selecione **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="55878-200">On the **Alerts** page, accept the default settings, and then select **Next**.</span></span>
8. <span data-ttu-id="55878-201">Na página **Resumo** , selecione **Avançar**e, em seguida, selecione **Fechar**.</span><span class="sxs-lookup"><span data-stu-id="55878-201">On the **Summary** page, select **Next**, and then select **Close**.</span></span>

## <a name="part-5-start-the-windows-10-upgrade-task-sequence"></a><span data-ttu-id="55878-202">Parte 5: Sequência de atualização de tarefa de iniciar o Windows 10</span><span class="sxs-lookup"><span data-stu-id="55878-202">Part 5: Start the Windows 10 upgrade task sequence</span></span>
<span data-ttu-id="55878-203">Siga estas etapas para iniciar a sequência de tarefas do Windows 10 Upgrade no dispositivo que você está atualizando.</span><span class="sxs-lookup"><span data-stu-id="55878-203">Follow these steps to start the Windows 10 Upgrade task sequence on the device that you are upgrading.</span></span>
 
1. <span data-ttu-id="55878-204">Faça logon no computador com o Windows e inicie o **Centro de Software**.</span><span class="sxs-lookup"><span data-stu-id="55878-204">Log on to the Windows computer and start **Software Center**.</span></span>
2. <span data-ttu-id="55878-205">Selecione a sequência de tarefas que você criou na etapa anterior e selecione **instalar**.</span><span class="sxs-lookup"><span data-stu-id="55878-205">Select the task sequence that you created in a previous step, and then select **Install**.</span></span>
3. <span data-ttu-id="55878-206">Quando começa a sequência de tarefas, ele iniciará automaticamente o processo de atualização in-loco invocando-se o programa de instalação do Windows (Setup.exe) com os parâmetros de linha de comando necessários para executar uma atualização automatizada, o que preserva a todos os dados, configurações, aplicativos, e drivers.</span><span class="sxs-lookup"><span data-stu-id="55878-206">When the task sequence begins, it automatically initiates the in-place upgrade process by invoking the Windows setup program (Setup.exe) with the necessary command-line parameters to perform an automated upgrade, which preserves all data, settings, apps, and drivers.</span></span>
4. <span data-ttu-id="55878-207">Depois que a sequência de tarefa for concluída com êxito, o computador será totalmente atualizado para Windows 10.</span><span class="sxs-lookup"><span data-stu-id="55878-207">After the task sequence completes successfully, the computer will be fully upgraded to Windows 10.</span></span>

<span data-ttu-id="55878-p118">Se você tiver problemas ao usar o Windows 10 em um ambiente empresarial, você pode consultar as [principais soluções de suporte da Microsoft para os problemas mais comuns](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). Esses recursos incluem artigos KB, atualizações e artigos da biblioteca.</span><span class="sxs-lookup"><span data-stu-id="55878-p118">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="55878-p119">Durante a distribuição das atualizações em sua organização, use o recurso de conformidade de atualização de análise do Windows para fornecer uma visão abrangente da conformidade da atualização, progresso de implantação da atualização e falha de solução de problemas para os dispositivos Windows 10 SO. Esse novo serviço usa dados de diagnóstico, incluindo o progresso da instalação, configuração do Windows Update e outras informações para fornecer tais ideias, sem nenhum custo extra e sem requisitos de infraestrutura adicional. Se ele é usado com o Windows Update para negócios ou outras ferramentas de gerenciamento, você pode ter certeza de que seus dispositivos estão atualizados corretamente.</span><span class="sxs-lookup"><span data-stu-id="55878-p119">During the rollout of updates across your organization, use the Update Compliance capability of Windows Analytics to provide a holistic view of OS update compliance, update deployment progress, and failure troubleshooting for Windows 10 devices. This new service uses diagnostic data including installation progress, Windows Update configuration and other information to provide such insights, at no extra cost and without additional infrastructure requirements. Whether it's used with Windows Update for Business or other management tools, you can be assured that your devices are properly updated.</span></span>

<span data-ttu-id="55878-213">Consulte [Monitor atualizações do Windows e antivírus do Windows Defender com a conformidade da atualização](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) para saber mais, introdução e usar a conformidade da atualização.</span><span class="sxs-lookup"><span data-stu-id="55878-213">See [Monitor Windows Updates and Windows Defender Antivirus with Update Compliance](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) to learn more, get started, and use Update Compliance.</span></span>

<span data-ttu-id="55878-214">Como ponto de verificação provisório, você pode consultar os [critérios de saída](windows10-exit-criteria.md#crit-windows10-step2) para esta etapa.</span><span class="sxs-lookup"><span data-stu-id="55878-214">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step2) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="55878-215">Próxima etapa</span><span class="sxs-lookup"><span data-stu-id="55878-215">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step3.png)| [<span data-ttu-id="55878-216">Implantar o Windows 10 Enterprise para novos dispositivos com piloto automático do Windows</span><span class="sxs-lookup"><span data-stu-id="55878-216">Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>](windows10-deploy-autopilot.md) |
