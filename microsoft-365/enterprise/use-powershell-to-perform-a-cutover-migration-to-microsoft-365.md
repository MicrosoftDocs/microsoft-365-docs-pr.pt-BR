...
Você pode migrar o conteúdo das caixas de correio de usuário de um sistema de email de origem para Microsoft 365 tudo de uma vez usando uma migração de transferência. Este artigo apresenta as tarefas para uma migração de substituição de email usando o PowerShell do Exchange Online.

Ao analisar o tópico, o que você precisa saber sobre [uma migração](/Exchange/mailbox-migration/what-to-know-about-a-cutover-migration)de email de recortamento para Microsoft 365 , você pode obter uma visão geral do processo de migração. Quando você estiver familiarizado com os conteúdos do artigo, use o seguinte para começar a migrar caixas de correio de um sistema de email para outro.

> [!NOTE]
> Você também pode usar o centro de administração Exchange para executar uma migração de recortamento. Consulte [Perform a cutover migration of email to Microsoft 365](/Exchange/mailbox-migration/cutover-migration-to-office-365).
...
- **Adicione sua organização local Exchange como um domínio aceito de sua organização Microsoft 365 local.** O serviço de migração usa o endereço SMTP de suas caixas de correio locais para criar Microsoft Online Services ID de usuário e endereço de email para as novas caixas de correio Microsoft 365 de usuário. A migração falhará se seu domínio Exchange não for um domínio aceito ou o domínio principal de sua Microsoft 365 organização. Para obter mais informações, consulte [Verify your domain](../admin/setup/add-domain.md).
...
- **Grupos de segurança e representantes** O serviço de migração de email não pode detectar se os grupos locais do Active Directory são grupos de segurança ou não, portanto, não pode provisioná-los como grupos de segurança no Microsoft 365. Se você quiser ter grupos de segurança em seu locatário Microsoft 365, primeiro provisione um grupo de segurança habilitado para email vazio em seu locatário Microsoft 365 antes de iniciar a migração de recortamento. Além disso, esse método de migração só move caixas de correio, usuários de email, contatos de email e grupos habilitados para email. Se qualquer outro objeto do Active Directory, como o usuário que não foi migrado para o Microsoft 365, for atribuído como gerente ou representante para um objeto que está sendo migrado, ele deverá ser removido do objeto antes de migrar.
...
Depois de alterar o registro MX e verificar se todos os emails estão sendo roteados para Microsoft 365 caixas de correio, notifique os usuários de que seus emails Microsoft 365. Depois disso, você pode excluir o lote de migração de substituição. Antes de excluir o lote de migração, verifique os itens a seguir.
 ...
 **Ative Microsoft 365 contas de usuário para as contas migradas atribuindo licenças.** Se você não atribuir uma licença, a caixa de correio será desabilitada quando terminar o período de carência (30 dias). Para atribuir uma licença no centro de administração Microsoft 365, consulte [Atribuir ou desatribuição de licenças](../admin/manage/assign-licenses-to-users.md).
...
- **Crie um registro DNS de Descoberta Automática para que os usuários possam acessar facilmente suas caixas de correio.** Depois que todas as caixas de correio locais são migradas para o Microsoft 365, você pode configurar um registro DNS de Descoberta Automática para sua organização Microsoft 365 para permitir que os usuários se conectem facilmente às suas novas caixas de correio Microsoft 365 com clientes Outlook e móveis. Esse novo registro DNS de Descoberta Automática precisa usar o mesmo namespace que você está usando para sua Microsoft 365 organização. Por exemplo, se seu namespace baseado na nuvem for cloud.contoso.com, o registro DNS de Descoberta Automática que você precisa criar será autodiscover.cloud.contoso.com.
    ...
    Se você manter seu Exchange Server, certifique-se de que o registro CNAME DNS de Descoberta Automática tenha que apontar para o Microsoft 365 no DNS interno e externo após a migração para que o cliente Outlook se conecte à caixa de correio correta.
