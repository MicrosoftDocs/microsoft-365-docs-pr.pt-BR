...
Você pode usar o ambiente resultante para testar os recursos e a funcionalidade do [Microsoft 365 for Enterprise](https://www.microsoft.com/microsoft-365/enterprise) com [guias de laboratório de teste](m365-enterprise-test-lab-guides.md) adicionais ou por conta própria.
...
Nesta fase, crie uma intranet simulada nos serviços de infraestrutura do Azure que inclui um controlador de domínio do AD DS (serviços de domínio Active Directory), um servidor de aplicativos e um computador cliente.

Você usará esses computadores nos [guias adicionais de laboratório de teste do Microsoft 365 para Enterprise](m365-enterprise-test-lab-guides.md) para configurar e demonstrar a identidade híbrida e outros recursos.
...
Neste método, você usa um modelo do Azure Resource Manager para criar a intranet simulada. Os modelos do Azure Resource Manager contêm todas as instruções para criar a infraestrutura de rede do Azure, as máquinas virtuais e sua configuração.

Antes de implantar o modelo, leia a [página Leiame do modelo](https://github.com/maxskunkworks/TLG/tree/master/tlg-base-config_3-vm.m365-ems) e prepare as seguintes informações:

- O nome de domínio DNS público do seu ambiente de teste (testlab. \<*your public domain*> ). Você inserirá esse nome no campo **Domain Name** da página de **implantação personalizada** .
...
>[!Note]
>A intranet simulada construída pelo modelo do Azure Resource Manager requer uma assinatura paga do Azure.

Após a conclusão do modelo, sua configuração terá a seguinte aparência:
...
Defina sua assinatura do Azure. Substitua tudo dentro das aspas, incluindo os colchetes angulares ("<" e ">") com o nome correto.
...
Crie o novo grupo de recursos com estes comandos. Substitua tudo dentro das aspas, incluindo os colchetes angulares, com os nomes corretos.
...
Em seguida, crie a rede virtual do TestLab que hospedará a sub-rede da rede corporativa do ambiente empresarial simulado e proteja-a com um grupo de segurança de rede. Preencha o nome do grupo de recursos e execute estes comandos no prompt de comando do PowerShell no computador local.
...
1. No [portal do Azure](https://portal.azure.com), selecione **grupos de recursos** > <***o nome do novo grupo de recursos***> > **DC1**  >  **conectar**.
    
2. No painel abrir, selecione **baixar arquivo RDP**. Abra o arquivo DC1. rdp baixado e selecione **conectar**.
    
3. Especifique o nome da conta de administrador local na DC1:
    
   - No Windows 7:
    
     Na caixa de diálogo **segurança do Windows** , selecione **usar outra conta**. Em **nome de usuário**, digite o nome da conta de ** \\ ** < *administrador local* do DC1>.
    
   - No Windows 8 ou Windows 10:
    
     Na caixa de diálogo **segurança do Windows** , selecione **mais opções**e selecione **usar uma conta diferente**. Em **nome de usuário**, digite o nome da conta de ** \\ ** < *administrador local* do DC1>.
    
4. Em **senha**, digite a senha da conta de administrador local e, em seguida, selecione **OK**.
...
1. No [portal do Azure](https://portal.azure.com), selecione **grupos de recursos** > <*o nome do grupo de recursos*> > **DC1**  >  **conectar**.
    
2. Execute o arquivo DC1. rdp baixado e selecione **conectar**.
    
3. Em **segurança do Windows**, selecione **usar outra conta**. Em **nome do usuário**, **digite \\ TESTLAB** < *nome da conta de administrador local*>.
    
4. Na caixa **senha** , digite a senha da conta de administrador local e, em seguida, selecione **OK**.
...
Observe que esse comando solicita que você forneça a senha da conta Usuário1. Essa conta será usada para conexões de área de trabalho remota para todos os computadores membros do domínio TESTLAB, portanto, escolha uma senha forte. Registre a senha da conta Usuário1 e armazene-a em um local seguro.
...
A configuração atual tem a seguinte aparência:
...
Observe que você depois de executar o comando **Add-Computer** , você deve fornecer as \\ credenciais de conta de domínio do Usuário1 TESTLAB.
...
Para criar uma máquina virtual do Azure para o CLIENT1, preencha o nome do grupo de recursos e execute estes comandos no prompt de comando do computador local.
...
2. Em **Propriedades de CLIENT1**, selecione ao lado da **configuração de segurança avançada** **do** IE.
    
3. Na **configuração de segurança avançada do Internet Explorer**, selecione **desativado** para **Administradores** e **usuários**e, em seguida, selecione **OK**.
    
4. Na tela iniciar, selecione **Internet Explorer**e, em seguida, selecione **OK**.
...
6. Na barra de tarefas da área de trabalho, selecione o ícone do explorador de arquivos.
    
7. Na barra de endereços, digite ** \\ \\ App1 \\ arquivos**e pressione **Enter**. Você verá uma janela de pasta com o conteúdo da pasta compartilhada arquivos.
...
- Use uma assinatura de produção separada do Microsoft 365 E5 com uma pequena quantidade de licenças.
  ...
  Esse é um custo adicional, mas garante que você tenha um ambiente de teste de trabalho que não expire; nele, você pode experimentar recursos, configurações e cenários. Você pode usar o mesmo ambiente de teste a longo prazo para provas de conceito, demonstração para colegas e gerenciamento e desenvolvimento e teste de aplicativos. Este é o método recomendado.
...
Sua configuração final tem a seguinte aparência:
