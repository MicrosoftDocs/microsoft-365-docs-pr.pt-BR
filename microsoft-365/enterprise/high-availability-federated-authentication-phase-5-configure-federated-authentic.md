...
Nesta fase final da implantação da autenticação federada de alta disponibilidade para o Microsoft 365 nos serviços de infraestrutura do Azure, você obterá e instalará um certificado emitido por uma autoridade de certificação pública, verificará sua configuração e, em seguida, instalará e executará o Azure AD Connect no servidor de sincronização de diretórios. O Azure AD Connect configura sua assinatura do Microsoft 365, seus Serviços de Federação do Active Directory (AD FS) e servidores proxy de aplicativo Web para autenticação federada.
  
Confira [Implantar a autenticação federada de alta disponibilidade para o Microsoft 365 no Azure](deploy-high-availability-federated-authentication-for-microsoft-365-in-azure.md) para todas as fases.
...
- A propriedade estendida Nome Alternativo da Assunto (SAN) é definida como seu FQDN do serviço de federação (exemplo: fs.contoso.com).
    
- O certificado deve ter a chave privada e ser armazenado no formato PFX.
    
Além disso, os computadores e dispositivos da sua organização devem confiar na autoridade de certificação pública que em emitido o certificado digital. Essa confiança é estabelecida com um certificado raiz da autoridade de certificação pública instalada no armazenamento de autoridades de certificação raiz confiáveis em seus computadores e dispositivos. Computadores que executam o Microsoft Windows normalmente têm um conjunto desses tipos de certificados instalados de autoridades de certificação comumente usadas. Se o certificado raiz de sua autoridade de certificação pública ainda não estiver instalado, você deverá implantá-lo nos computadores e dispositivos da sua organização.
  
Para obter mais informações sobre requisitos de certificado para autenticação federada, consulte [Pré-requisitos para instalação e configuração de federação.](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-prerequisites#prerequisites-for-federation-installation-and-configuration)
  
Quando você receber o certificado, copie-o para uma pasta na unidade C: do servidor de sincronização de diretórios. Por exemplo, nomee o arquivo SSL.pfx e armazene-o na pasta C: Certs no \\ servidor de sincronização de diretórios.
...
- Você determinou um FQDN do serviço de federação com base em seu nome de domínio público.
    
- Um registro DNS A público para o FQDN do serviço de federação aponta para o endereço IP público do balanceador de carga do Azure voltado para a Internet para os servidores proxy de aplicativos Web.
...
- Um certificado digital emitido pela autoridade de certificação pública adequado para conexões SSL com o SAN definido para seu FQDN do serviço de federação é um arquivo PFX armazenado no servidor de sincronização de diretórios.
    
- O certificado raiz da autoridade de certificação pública é instalado no armazenamento autoridades de certificação raiz confiáveis em seus computadores e dispositivos.
...
A ferramenta Azure AD Connect configura os servidores do AD FS, os servidores proxy de aplicativo Web e o Microsoft 365 para autenticação federada com estas etapas:
  
1. Crie uma conexão de área de trabalho remota com seu servidor de sincronização de diretórios com uma conta de domínio que tenha privilégios de administrador local.
...
4. Na página **Bem-vindo ao Azure AD Connect,** clique em **Concordar** e em **Continuar.**
...
9. Na  página Conectar seus diretórios, verifique se sua floresta local dos Serviços de Domínio Active Directory (AD DS) está selecionada na **floresta,** digite o nome e a senha de uma conta de administrador de domínio, clique em **Adicionar** Diretório e clique em **Próximo.**
    
10. Na página de configuração de entrada do **Azure AD,** clique em **Próximo.**
    
11. Na página **De domínio e filtragem de UO,** clique em **Próximo**.
    
12. Na página **Identificando exclusivamente seus usuários,** clique em **Próximo.**
...
18. Verifique se o **Nome do Assunto** e o Nome do **Serviço** de Federação estão definidos para o FQDN do serviço de federação e clique em **Próximo.**
    
19. Na página de servidores **do AD FS,** digite o nome do seu primeiro servidor do AD FS (Tabela M - Item 4 - Coluna de nome da máquina virtual) e clique em **Adicionar**.
    
20. Digite o nome do seu segundo servidor do AD FS (Tabela M - Item 5 - coluna de nome da máquina virtual), clique em **Adicionar** e em **Próximo.**
    
21. Na página servidores proxy de aplicativo **Web,** digite o nome do seu primeiro servidor proxy de aplicativo Web (Tabela M - Item 6 - coluna de nome da máquina virtual) e clique em **Adicionar**.
    
22. Digite o nome do seu segundo servidor proxy de aplicativo Web (Tabela M - Item 7 - coluna de nome da máquina virtual), clique em **Adicionar** e clique em **Próximo**.
    
23. Na página **de credenciais do Administrador de** Domínio, digite o nome de usuário e a senha de uma conta de administrador de domínio e clique em **Próximo.**
    
24. Na página **da conta de serviço do AD FS,** digite o nome de usuário e a senha de uma conta de administrador corporativo e clique em **Próximo.**
...
27. Na página **Instalação Completa**, clique em **Verificar**. Você verá duas mensagens indicando que a configuração da intranet e da Internet foi verificada com êxito.
    
  - A mensagem da intranet deve listar o endereço IP privado do seu balanceador de carga interno do Azure para seus servidores do AD FS.
    
  - A mensagem da Internet deve listar o endereço IP público do seu balanceador de carga voltado para a Internet do Azure para seus servidores proxy de aplicativo Web.
