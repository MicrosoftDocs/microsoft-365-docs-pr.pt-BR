...
O envio **do comentário** está disponível com ou sem a atualização de propriedades.

As propriedades atualizáveis são: `status` `determination` , e `classification` `assignedTo` .
...
status | String | Especifica o status atualizado dos alertas especificados. Os valores da propriedade são: 'New', 'InProgress' e 'Resolved'.
assignedTo | String | Proprietário dos alertas especificados
...
determinação | String | Especifica a determinação dos alertas especificados. Os valores da propriedade são: 'NotAvailable', 'Apt', 'Malware', 'SecurityPersonnel', 'SecurityTesting', 'UnwantedSoftware', 'Other'
