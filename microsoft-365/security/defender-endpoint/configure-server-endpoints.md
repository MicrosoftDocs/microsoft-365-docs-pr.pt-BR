...
---
# <a name="onboard-windows-servers-to-the-microsoft-defender-for-endpoint-service"></a>Integração Windows servidores para o serviço do Microsoft Defender para Ponto de Extremidade
...
- Windows Server 2016
- Windows Servidor (SAC) versão 1803 e posterior
- Windows Server 2019 e posterior
- Windows Edição principal do Server 2019
...
O Defender para Ponto de Extremidade estende o suporte para incluir também o sistema operacional Windows Server. Esse suporte fornece recursos avançados de detecção e investigação de ataques perfeitamente por meio do Microsoft 365 Defender console.

Para obter uma orientação prática sobre o que precisa ser feito para licenciamento e infraestrutura, consulte [Protecting Windows Servers with Defender for Endpoint](https://techcommunity.microsoft.com/t5/What-s-New/Protecting-Windows-Server-with-Windows-Defender-ATP/m-p/267114#M128).

Para obter orientações sobre como baixar e usar linhas de base Segurança do Windows para servidores Windows, consulte [Segurança do Windows Baselines](/windows/device-security/windows-security-baselines).
...
Você pode Windows o Windows Server 2008 R2 SP1, Windows Server 2012 R2 e Windows Server 2016 para o Defender para Ponto de Extremidade usando qualquer uma das seguintes opções:

- **Opção 1**: [Integração instalando e configurando Microsoft Monitoring Agent (MMA)](#option-1-onboard-by-installing-and-configuring-microsoft-monitoring-agent-mma)
...
- **Opção 3**: [Integração Microsoft Endpoint Manager versão 2002 e posterior](#option-3-onboard-windows-servers-through-microsoft-endpoint-manager-version-2002-and-later)

Após concluir as etapas de integração usando qualquer uma das opções fornecidas, você precisará configurar e atualizar System Center Endpoint Protection [clientes](#configure-and-update-system-center-endpoint-protection-clients).

> [!NOTE]
> A licença de servidor autônomo do Defender for Endpoint é necessária, por nó, para a integração de um servidor Windows por meio da Microsoft Monitoring Agent (Opção 1) ou por meio Microsoft Endpoint Manager (Opção 3). Como alternativa, uma licença do Azure Defender for Servers é necessária, por nó, para a integração de um servidor Windows por meio do Centro de Segurança do Azure (Opção 2), consulte Recursos com suporte disponíveis no [Azure Defender](/azure/security-center/security-center-services).

### <a name="option-1-onboard-by-installing-and-configuring-microsoft-monitoring-agent-mma"></a>Opção 1: Integrando instalando e configurando Microsoft Monitoring Agent (MMA)

Você precisará instalar e configurar o MMA para servidores Windows relatar dados do sensor ao Defender para o Ponto de Extremidade. Para obter mais informações, consulte [Coletar dados de log com o agente do Azure Log Analytics.](/azure/azure-monitor/platform/log-analytics-agent)

Se você já estiver usando o System Center SCOM (Operations Manager) ou o Azure Monitor (anteriormente conhecido como Operations Management Suite (OMS),anexe o Microsoft Monitoring Agent (MMA) para relatar ao seu espaço de trabalho do Defender for Endpoint por meio do suporte a Multihoming.
...
2. Ativar o monitoramento do servidor a partir Microsoft 365 Defender portal.
3. Instale e configure o MMA para o servidor relatar dados do sensor ao Defender para Ponto de Extremidade.
4. Configure e atualize System Center Endpoint Protection clientes.
...
Para Windows Server 2008 R2 SP1, certifique-se de atender aos seguintes requisitos:
...
Para Windows Server 2008 R2 SP1 e Windows Server 2012 R2: Configurar e atualizar [System Center Endpoint Protection clientes](#configure-and-update-system-center-endpoint-protection-clients).

> [!NOTE]
> Esta etapa só será necessária se sua organização usar System Center Endpoint Protection (SCEP) e você estiver integrando o Windows Server 2008 R2 SP1 e Windows Server 2012 R2.
...
1. Baixe o arquivo de instalação do agente: [Windows agente de 64 bits.](https://go.microsoft.com/fwlink/?LinkId=828603)

2. Usando a ID do Workspace e a tecla Workspace obtidas no procedimento anterior, escolha qualquer um dos seguintes métodos de instalação para instalar o agente no servidor Windows:
    - [Instale manualmente o agente usando a instalação](/azure/log-analytics/log-analytics-windows-agents#install-agent-using-setup-wizard). 
    Na página **Opções de Instalação do** Agente, escolha Conexão o agente para **OMS (Análise de Log do Azure).**
...
### <a name="configure-windows-server-proxy-and-internet-connectivity-settings-if-needed"></a>Configurar Windows proxy de servidor e configurações de conectividade com a Internet, se necessário
...
- [Configurar o MMA para usar um servidor proxy](/azure/azure-monitor/platform/agent-windows#install-agent-using-setup-wizard)

- [Configurar Windows usar um servidor proxy para todas as conexões](configure-proxy-internet.md)
...
Depois de concluído, você deverá ver servidores Windows no portal dentro de uma hora.

### <a name="option-2-onboard-windows-servers-through-azure-security-center"></a>Opção 2: Integração Windows por meio do Centro de Segurança do Azure

1. No painel Microsoft 365 Defender de navegação, selecione **Configurações**  >  **Endpoints**  >  **Gerenciamento de**  >  **dispositivos Integrando**.
...
Após concluir as etapas de integração, você precisará configurar e atualizar System Center Endpoint Protection [clientes.](#configure-and-update-system-center-endpoint-protection-clients)

> [!NOTE]
>
> - Para que a integração por meio do Azure Defender for Servers funcione conforme o esperado, o servidor deve ter um espaço de trabalho e uma chave apropriados configurados nas configurações Microsoft Monitoring Agent (MMA).
...
### <a name="option-3-onboard-windows-servers-through-microsoft-endpoint-manager-version-2002-and-later"></a>Opção 3: Integração Windows servidores por meio Microsoft Endpoint Manager versão 2002 e posterior

Você pode Windows Server 2012 R2 e Windows Server 2016 usando Microsoft Endpoint Manager versão 2002 e posterior. Para obter mais informações, consulte [Microsoft Defender for Endpoint in Microsoft Endpoint Manager branch atual](/mem/configmgr/protect/deploy-use/defender-advanced-threat-protection).

Após concluir as etapas de integração, você precisará configurar e atualizar System Center Endpoint Protection [clientes.](#configure-and-update-system-center-endpoint-protection-clients)

## <a name="windows-server-sac-version-1803-windows-server-2019-and-windows-server-2019-core-edition"></a>Windows Servidor (SAC) versão 1803, Windows Server 2019 e Windows Server 2019 Core edition

Você pode integrar o Windows Server (SAC) versão 1803, Windows Server 2019 ou Windows Server 2019 Core edition usando os seguintes métodos de implantação:
...
> [!NOTE]
>
> - O pacote onboarding do Windows Server 2019 por meio Microsoft Endpoint Manager atualmente envia um script. Para obter mais informações sobre como implantar scripts no Configuration Manager, consulte [Pacotes e programas no Configuration Manager](/configmgr/apps/deploy-use/packages-and-programs).
> - Um script local é adequado para uma prova de conceito, mas não deve ser usado para implantação de produção. Para uma implantação de produção, recomendamos usar a Política de Grupo ou Microsoft Endpoint Configuration Manager.

O suporte para Windows Server fornece informações mais profundas sobre atividades do servidor, cobertura para detecção de ataque de kernel e memória e habilita ações de resposta.

1. Configure as configurações de integração do Defender para Ponto de Extremidade no servidor Windows usando as mesmas ferramentas e métodos para Windows 10 dispositivos. Para obter mais informações, consulte [Onboard Windows 10 devices](configure-endpoints.md).
...
O Defender for Endpoint pode se integrar ao Azure Defender para fornecer uma solução abrangente Windows proteção do servidor. Com essa integração, o Azure Defender pode usar o poder do Defender para o Ponto de Extremidade para fornecer detecção aprimorada de ameaças para servidores Windows.

Os seguintes recursos estão incluídos nesta integração:

- Integração automatizada - o sensor Defender para Ponto de Extremidade é habilitado automaticamente em servidores Windows que estão integrados ao Azure Defender. Para obter mais informações sobre a integração do Azure Defender, [consulte Use the integrated Microsoft Defender for Endpoint license](/azure/security-center/security-center-wdatp).
...
- Windows servidores monitorados pelo Azure Defender também estarão disponíveis no Defender para Ponto de Extremidade - o Azure Defender conecta-se perfeitamente ao locatário do Defender para Ponto de Extremidade, fornecendo uma exibição única entre clientes e servidores.  Além disso, os alertas do Defender para Ponto de Extremidade estarão disponíveis no console do Azure Defender.
- Investigação do servidor - Os clientes do Azure Defender podem acessar Microsoft 365 Defender portal para executar uma investigação detalhada para descobrir o escopo de uma possível violação.
...
O monitoramento do ponto de extremidade do servidor que utiliza essa integração foi desabilitado para Office 365 GCC clientes.
...
O Defender para Ponto de Extremidade se integra ao System Center Endpoint Protection. A integração fornece visibilidade para detecções de malware e para interromper a propagação de um ataque em sua organização, proibindo arquivos potencialmente mal-intencionados ou malwares suspeitos.

As etapas a seguir são necessárias para habilitar essa integração:

- Instale a atualização da plataforma anti-malware de janeiro de [2017 para Endpoint Protection clientes](https://support.microsoft.com/help/3209361/january-2017-anti-malware-platform-update-for-endpoint-protection-clie).

- [Configure a associação do Serviço de Proteção de Nuvem](/windows/security/threat-protection/microsoft-defender-antivirus/enable-cloud-protection-microsoft-defender-antivirus) do cliente SCEP à **configuração** Avançado.

## <a name="offboard-windows-servers"></a>Servidores Windows offboard

Você pode offboard Windows Server (SAC), Windows Server 2019 e Windows Server 2019 Core edition no mesmo método disponível para dispositivos Windows 10 cliente.

Para outras Windows de servidor, você tem duas opções para sair Windows servidores do serviço:
...
### <a name="uninstall-windows-servers-by-uninstalling-the-mma-agent"></a>Desinstalar Windows servidores desinstalando o agente MMA
...
Para obter mais informações, consulte [Para desabilitar um agente](/azure/log-analytics/log-analytics-windows-agents#to-disable-an-agent).
...
Para tirar o Windows servidor, você pode usar um dos seguintes métodos:
...
1. Na Microsoft Monitoring Agent **Propriedades**, selecione a guia Análise de **Log do Azure (OMS).**
...
1. Obter sua ID do Workspace:
   ...
   1. Selecione **Windows Server 2008 R2 SP1, 2012 R2 e 2016** como o sistema operacional e obter sua ID do Espaço de Trabalho:
...
Para os dois KBs (um para o Windows Server 2008R2/Windows 7 e o outro para Windows Server 2012 R2) repetir o processo, mas criar direcionamento de nível de item na guia COMMON, portanto, o arquivo só será copiado para a versão apropriada da plataforma/sistema operacional no escopo:

:::image type="content" source="images/targeteditor.png" alt-text="editor de destino":::

- Para Windows Server 2008 R2, você precisa (e ele só copiará) Windows6.1-BJ3080149-x64.msu
- Para Windows Server 2012 R2 você precisa (e ele só copiará) Windows8.1-BJ3080149-x64.msu
...
Para Windows Server 2008 R2 PS1, certifique-se de atender aos seguintes requisitos:

- Instalar a atualização mensal de fevereiro de [2018](https://support.microsoft.com/help/4074598/windows-7-update-kb4074598)
  
- Instalar o [.NET framework 4.5](https://www.microsoft.com/download/details.aspx?id=30653) (ou posterior) ou [KB3154518](https://support.microsoft.com/help/3154518/support-for-tls-system-default-versions-included-in-the-net-framework)

Verifique se os KBs estão presentes antes da integração do Windows Server 2008 R2 Esse processo permite que você aborde todos os servidores se você não tiver o Configuration Manager gerenciando servidores.
