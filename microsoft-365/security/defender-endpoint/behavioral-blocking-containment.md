...
- **Regras de redução [](attack-surface-reduction.md)** de superfície de ataque orientadas por política no cliente Comportamentos de ataque comuns predefinidos são impedidos de executar, de acordo com suas regras de redução de superfície de ataque. Quando esses comportamentos tentam ser executados, eles podem ser vistos Microsoft 365 Defender ( [https://security.microsoft.com](https://security.microsoft.com) ) como alertas informacionais. As regras de redução de superfície de ataque não estão habilitadas por padrão; você configura suas políticas no [Microsoft 365 Defender](microsoft-defender-security-center.md).
