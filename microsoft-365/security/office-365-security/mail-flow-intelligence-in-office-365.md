...
Normalmente, isso significa Microsoft 365 não pode se conectar ao servidor de email de destino. Os detalhes do erro explicarão o problema. Por exemplo:

- Seu servidor de email local está inoossado.

- Há um erro nas configurações de host inteligente do conector, portanto, Microsoft 365 está tentando se conectar ao endereço IP errado.
...
Normalmente, esse erro significa Microsoft 365 encontrou um erro de conexão ao tentar se conectar ao servidor de email de destino. Uma causa provável para esse erro é que o firewall está bloqueando conexões Microsoft 365 endereços IP. Ou, esse erro pode ser por design se você tiver migrado completamente seu sistema de email local para Microsoft 365 e desligar seu ambiente de email local.

### <a name="how-do-i-fix-error-code-450-44316"></a>Como corrigir o código de erro 450 4.4.316?

- Se você tiver caixas de correio em seu ambiente local, precisará modificar suas configurações de firewall para permitir conexões Microsoft 365 endereços IP na porta TCP 25 para seus servidores de email locais. Para ver uma lista dos endereços IP Microsoft 365, consulte [Microsoft 365 URLs e intervalos de endereços IP.](../../enterprise/urls-and-ip-address-ranges.md)
...
- Se nenhuma mensagem mais deve ser entregue ao seu ambiente local, clique em **Corrigir** agora no alerta para que Microsoft 365 possa rejeitar imediatamente as mensagens com destinatários inválidos. Isso reduzirá o risco de exceder a cota da sua organização para destinatários inválidos, o que pode afetar a entrega normal de mensagens. Ou você pode usar as instruções a seguir para corrigir manualmente o problema:
  ...
  - No centro de administração [Exchange (EAC),](/Exchange/exchange-admin-center)desabilite ou exclua o conector que fornece emails do Microsoft 365 para seu ambiente de email local:

    1. No EAC, vá para **Conectores de fluxo** \> **de emails.**

    2. Selecione o conector com o **valor De** **Office 365** e o servidor de email de **Sua** organização e faça uma das seguintes etapas: 
  ...
  - Altere o domínio aceito em Microsoft 365 que está associado ao seu ambiente de email local de **Retransmissão** Interna para **Autoritativa**. Para obter instruções, [consulte Gerenciar domínios aceitos em Exchange Online](/exchange/mail-flow-best-practices/manage-accepted-domains/manage-accepted-domains).
...
Normalmente, esse erro significa Microsoft 365 conectado ao servidor de email de destino, mas o servidor respondeu com um erro imediato ou não responde aos requisitos de conexão. Os detalhes do erro explicarão o problema. Por exemplo:

- O servidor de email de destino respondeu com um erro "Serviço não disponível", que indica que o servidor não consegue manter a comunicação com Microsoft 365.
...
Normalmente, esse erro significa que Microsoft 365 está tendo dificuldades para se comunicar com seu ambiente de email local, portanto, a conexão foi retirada. As possíveis causas desse erro são:

- O firewall usa regras de exame de pacotes SMTP e essas regras não estão funcionando corretamente.
- Seu servidor de email local não está funcionando corretamente (por exemplo, travamentos de serviço, falhas ou recursos de sistema baixos), o que está fazendo com que o servidor se desajuste e feche a conexão com Microsoft 365.
- Há problemas de rede entre seu ambiente local e Microsoft 365.
...
- Se o problema for causado por problemas de rede entre seu ambiente local e Microsoft 365, contate sua equipe de rede para solucionar o problema.
- Se o erro for da sua organização parceira (por exemplo, um provedor de serviços de nuvem de terceiros), você precisará entrar em contato com seu parceiro para corrigir o problema.

## <a name="error-code-450-47320-certificate-validation-failed"></a>Código de erro: falha na validação do certificado 450 4.7.320

Normalmente, esse erro significa Microsoft 365 encontrou um erro ao tentar validar o certificado do servidor de email de destino. Os detalhes do erro explicarão o erro. Por exemplo:
...
- Correção do certificado ou das configurações no conector para que as mensagens em fila Microsoft 365 possam ser entregues.
- Se o erro for da sua organização parceira (por exemplo, um provedor de serviços de nuvem de terceiros), você precisará entrar em contato com seu parceiro para corrigir o problema.

## <a name="other-error-codes"></a>Outros códigos de erro

Microsoft 365 está tendo dificuldades para entregar mensagens para seu servidor de email local ou parceiro. Use as **informações do servidor** de destino no erro para examinar o problema em seu ambiente ou modificar o conector se houver um erro de configuração.
