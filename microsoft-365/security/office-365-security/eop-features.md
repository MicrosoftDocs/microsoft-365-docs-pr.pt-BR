...
|Gerenciar spam por Outlook ou Outlook na Web (anteriormente conhecido como Outlook Web App)|Os administradores e usuários finais podem criar listas de remetentes confiáveis e listas de remetentes bloqueados. Para obter mais informações, consulte [Sobre configurações de lixo eletrônico em Outlook](configure-junk-email-settings-on-exo-mailboxes.md#about-junk-email-settings-in-outlook). <p> Se você estiver usando o EOP para ajudar a proteger caixas de correio locais, use a sincronização de diretórios para ajudar a garantir que essas configurações sejam sincronizadas com o serviço. Para obter mais informações sobre como configurar a sincronização de diretórios, consulte "Use directory synchronization to manage mail users" in [Manage mail users in standalone EOP](/exchange/standalone-eop/manage-mail-users-in-eop).|
...
|Filtragem e ações baseadas em política|As políticas personalizadas são baseadas Exchange regras de fluxo de emails (também conhecidas como regras de transporte). Você pode filtrar por domínio, palavra-chave, nome do arquivo, tipo de arquivo, linha de assunto, corpo da mensagem, remetente, destinatário, cabeçalho e endereço IP. Para obter mais informações, consulte [Mail flow rules (transport rules) in Proteção do Exchange Online](mail-flow-rules-transport-rules-0.md).|
...
|**Administração**||
|Administração com base na Web|A maioria dos recursos é gerenciada no [Centro de Conformidade & Segurança.](grant-access-to-the-security-and-compliance-center.md) <p> Outros recursos exigem gerenciamento no Exchange de administração (EAC). Para obter mais informações, [consulte Exchange centro](/exchange/exchange-admin-center) de administração no Exchange Online ou Exchange de administração no [EOP](/exchange/standalone-eop/exchange-admin-center-eop)autônomo.|
|Sincronização de diretório|A sincronização de diretório está disponível por meio do Ferramenta de Sincronização do Microsoft Azure Active Directory. Para obter mais informações, consulte a seção "Usar a sincronização de diretórios para gerenciar usuários de email" em Gerenciar usuários de [email no EOP autônomo](/exchange/standalone-eop/manage-mail-users-in-eop).|
|Bloqueio de borda baseado em diretório (DBEB)|O recurso DBEB permite rejeitar mensagens de destinatários inválidos no perímetro da rede do serviço. O DBEB permite que os administradores adicionem destinatários habilitados para email Microsoft 365 e bloqueiem todas as mensagens enviadas para endereços de email que não estão presentes Microsoft 365. Para obter mais informações sobre como configurar o DBEB, consulte [Use Directory Based Edge Blocking to reject messages sent to invalid recipients](/exchange/mail-flow-best-practices/use-directory-based-edge-blocking).|
|PowerShell|A funcionalidade completa do EOP está disponível no EOP PowerShell autônomo. Para obter mais informações, [consulte Proteção do Exchange Online PowerShell](/powershell/exchange/exchange-online-protection-powershell).|
