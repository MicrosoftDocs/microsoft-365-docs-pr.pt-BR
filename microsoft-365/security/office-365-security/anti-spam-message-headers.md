...
|compauth|Resultado da autenticação composta. Usado pelo Office 365 para combinar vários tipos de autenticação, como SPF, DKIM, DMARC ou qualquer outra parte da mensagem, para determinar se a mensagem é autenticada ou não. Usa o domínio De: como base de avaliação.<br/><br/>Os valores possíveis incluem:<ul><li>**pass**: A mensagem passou na autenticação explícita (passou ou [na melhor estimativa passou](https://blogs.msdn.microsoft.com/tzink/2015/05/06/what-is-dmarc-bestguesspass-in-office-365) no DMARC) ou passou na autenticação implícita (o domínio de envio não publica registros de autenticação de email, mas o Office 365 possui fortes sinais de back-end para indicar que a mensagem provavelmente é legítima com alta confiabilidade).</li><li>**sofpass**: A mensagem transmitiu autenticação implícita com confiança baixa a média. O domínio de envio não publica registros de autenticação de email, mas o Office 365 possui sinais de back-end para indicar que a mensagem é legítima, mas a intensidade do sinal é mais fraca.</li><li>**fail**: A mensagem foi reprovada na autenticação explícita (o domínio de envio tem registros de autenticação de email) ou foi reprovada na autenticação implícita (o domínio de envio não tem registros de autenticação de email, portanto, o Office 365 interpolou o resultado como tendo registros publicados).</li><li>**nenhum**: A mensagem não foi autenticada (ou foi autenticada, mas não foi alinhada), mas a autenticação composta não foi aplicada devido à reputação do remetente ou a outros fatores.</li></ul>|
