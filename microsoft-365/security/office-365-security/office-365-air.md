...
> [!TIP]
> Para obter uma visão geral mais detalhada, confira [como funciona o Air](automated-investigation-response-office.md).

## <a name="how-to-get-air"></a>Como obter o AIR

Os recursos de ar estão incluídos no [Microsoft defender para Office 365](office-365-atp.md#microsoft-defender-for-office-365-plan-1-and-plan-2), desde que suas políticas e alertas estejam configurados. Se você quiser alguma ajuda com isso, siga as orientações em [proteção contra ameaças](protect-against-threats.md) para definir ou definir as seguintes configurações de proteção:
...
3. [Proteção contra phishing](protect-against-threats.md#part-2---anti-phishing-protection)
...
6. [Anexos seguros para o SharePoint, o onedrive e o Microsoft Teams](protect-against-threats.md#part-5---verify-atp-for-sharepoint-onedrive-and-microsoft-teams-is-turned-on).

7. [Limpeza automática de zero horas para email](protect-against-threats.md#zero-hour-auto-purge-for-email-in-eop).
...
|---|---|---|
|Um clique em URL potencialmente mal-intencionado foi detectado|**High**|Este alerta é gerado quando qualquer um dos seguintes ocorre: <ul><li>Um usuário protegido por [links seguros](atp-safe-links.md) em sua organização clica em um link mal-intencionado</li><li>As alterações de veredicto para URLs são identificadas pelo Microsoft defender para Office 365</li><li>Os usuários substituem as páginas de aviso de links seguros (com base na [política de links seguros](set-up-atp-safe-links-policies.md)da sua organização).</li></ul> <p> Para obter mais informações sobre eventos que disparam esse alerta, consulte [set up Safe links Policies](set-up-atp-safe-links-policies.md).|
|Uma mensagem de email é reportada por um usuário como malware ou Phish|**Informativa**|Este alerta é gerado quando os usuários na sua organização relatam mensagens como email de phishing usando o [suplemento de mensagem de relatório](enable-the-report-message-add-in.md).|
|Mensagens de email com malware são removidos após a entrega|**Informativa**|Esse alerta é gerado quando qualquer mensagem de email contendo malware é entregue às caixas de correio em sua organização. Se esse evento ocorrer, a Microsoft removerá as mensagens infectadas das caixas de correio do Exchange Online usando a [limpeza automática de zero hora](zero-hour-auto-purge.md).|
|Mensagens de email que contêm URLs de phishing são removidas após a entrega|**Informativa**|Esse alerta é gerado quando qualquer mensagem que contenha Phish é entregue às caixas de correio em sua organização. Se esse evento ocorrer, a Microsoft removerá as mensagens infectadas das caixas de correio do Exchange Online usando a [limpeza automática de zero hora](zero-hour-auto-purge.md).|
|Padrões de envio de emails suspeitos detectados|**Medium**|Este alerta é gerado quando alguém em sua organização enviou emails suspeitos e está correndo o risco de ser restrito a enviar emails. Este é um aviso antecipado para comportamento que pode indicar que a conta está comprometida, mas não grave o suficiente para restringir o usuário. <p> Embora seja raro, um alerta gerado por essa política pode ser uma anomalia. No entanto, é uma boa ideia [verificar se a conta do usuário está comprometida](responding-to-a-compromised-email-account.md).|
|Um usuário não está restringindo o envio de email|**High**|Este alerta é gerado quando alguém em sua organização não tem permissão para enviar emails de saída. Isso geralmente ocorre quando uma [conta de email é comprometida](responding-to-a-compromised-email-account.md). <p> Para obter mais informações sobre usuários restritos, consulte [remover usuários bloqueados do portal de usuários restritos no Microsoft 365](removing-user-from-restricted-users-portal-after-spam.md).|
|
...
As licenças do [Microsoft defender for Office 365 plano 2](office-365-atp.md#microsoft-defender-for-office-365-plan-1-and-plan-2) devem ser atribuídas a:
...
- [Veja detalhes e resultados de uma investigação automatizada](air-view-investigation-results.md#view-details-of-an-investigation)
