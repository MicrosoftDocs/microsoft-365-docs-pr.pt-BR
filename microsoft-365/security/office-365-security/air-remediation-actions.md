...
|User|Encaminhamento de e-mail <br> (As regras de encaminhamento de caixa de correio são configuradas, chch pode ser usado para exfiltração de dados.)|Remover regra de encaminhamento <p> Use [insights de fluxo de](mail-flow-insights-v2.md)emails , incluindo o [relatório](mfi-auto-forwarded-messages-report.md)de mensagens com foco automático, para exibir detalhes mais específicos sobre emails encaminhados.|
|User|Regras de delegação de email <br> (A conta de um usuário tem delegações configuradas.)|Remover regra de delegação <p> Se sua organização estiver usando [o Microsoft Defender para Ponto](/windows/security/threat-protection/)de Extremidade, considere investigar o [usuário](/microsoft-365/security/defender-endpoint/investigate-user) que está recebendo a permissão de delegação.|
|User|Exfiltração dos dados <br> (Um usuário violou políticas de [DLP](../../compliance/dlp-learn-about-dlp.md) de email ou compartilhamento de arquivos |A investigação automatizada não resulta em uma ação pendente específica. <p> [Exibir relatórios de DLP e tomar medidas](../../compliance/view-the-dlp-reports.md).|
