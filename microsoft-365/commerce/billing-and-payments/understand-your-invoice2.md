...
## <a name="understand-billing-concepts"></a>Entenda os conceitos de cobrança

Antes de exibir a fatura, é preciso entender alguns conceitos de cobrança.
  
### <a name="invoice-balance-vs-order-balance"></a>Saldo da fatura versus saldo do pedido

 O **saldo da fatura** é o valor mostrado na fatura. Trata-se apenas do valor devido por um determinado período de cobrança. O **saldo total do pedido** é a soma de todas as faturas não pagas. Você pode exibir o saldo do pedido na seção **Cobrança** do Centro de Administração.
  
### <a name="billing-frequency-vs-invoice-frequency"></a>Frequência de cobrança versus frequência de faturamento

 A **frequência de cobrança** significa quantas vezes você é cobrado. As assinaturas são cobradas mensal ou anualmente, dependendo de qual opção você selecionou durante a aquisição da assinatura. A **frequência da fatura** significa quantas vezes você recebe uma fatura. Se você escolheu a cobrança anual, só receberá uma fatura por ano, a menos que as atividades da sua assinatura exijam uma cobrança ou um crédito.
...
|**Nome**|**Descrição**|
|:-----|:-----|
|Preço anual|As assinaturas são cobradas mensalmente ou anualmente. Quando você opta pela cobrança anual durante a aquisição da assinatura, o preço anual da licença se reflete na fatura. Caso pretenda alterar a frequência de cobrança, cancele a assinatura e compre-a novamente com a nova frequência de cobrança.|
...
|Enviar Cobrança para|Esse é o endereço do departamento de cobrança, e normalmente, é o mesmo endereço do campo Vendido Para. Para atualizar o endereço de cobrança, confira o artigo [Alterar seu endereço de cobrança](change-your-billing-addresses.md).|
...
|Número da OC do Cliente|O número da OC (ordem de compra). Quando você atualizar o número da OC, as próximas faturas vão inclui-lo. [Mudar o número da OC](#change-your-purchase-order-number).  <br/> **NOTA** Você não pode adicionar um número de OC em uma fatura existente.           |
...
|Data de Vencimento|A data de vencimento do pagamento da fatura. Quando a assinatura é paga com cartão de crédito ou por débito em conta, cobramos essa forma de pagamento no dia seguinte à data da fatura.<br/> **NOTA** O pagamento por meio de conta bancária não está disponível em alguns países ou regiões.           |
|Transferência Eletrônica de Fundos|Quando você seleciona "fatura" como método de pagamento da assinatura, a página 1 inclui as informações da conta bancária da Microsoft para pagamentos eletrônicos (ACH, SEPA, etc). Normalmente, seu banco terá um campo de referência que deverá ser preenchido ao enviar um pagamento. Faça referência ao número da fatura exibido nesse campo.|
...
|Preço Mensal|As assinaturas são cobradas mensalmente ou anualmente. Quando você opta pela cobrança mensal durante a aquisição da assinatura, o preço mensal da licença se reflete na fatura. Caso pretenda alterar a frequência de cobrança, cancele a assinatura e compre-a novamente com a nova frequência de cobrança.|
